...
class BrowserTalentSiteAdapter(StaticTalentSiteAdapter):
    ...
    def _extract_text_jobs_from_soup(self, soup: BeautifulSoup) -> list[RecruitmentJob]:
        ...
        for index, line in enumerate(soup.get_text("\n", strip=True).splitlines(), start=1):
            ...
            # LLM enhancement for text-only mode; use analyze_document for consistency
            if llm and llm.available and len(description) >= 60:
                ...
                try:
                    ...
                    if llm_positions:
                        ...
                        if not department and extracted.get("department"):
                            department = extracted["department"]
                        if not discipline and disc:
                            discipline = disc
                        if not education_requirement and extracted.get("education_requirement"):
                            education_requirement = extracted["education_requirement"]
                        if not job_type and extracted.get("job_type"):
