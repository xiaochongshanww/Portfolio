    ...
    with engine.connect() as conn:
        ...
        try:
            charset = conn.execute(
                text("SHOW VARIABLES LIKE 'character_set_server'")
            ).fetchone()
            coll = conn.execute(
                text("SHOW VARIABLES LIKE 'collation_server'")
            ).fetchone()
            if charset:
                res["variables"]["character_set_server"] = charset[1]
            if coll:
                res["variables"]["collation_server"] = coll[1]
