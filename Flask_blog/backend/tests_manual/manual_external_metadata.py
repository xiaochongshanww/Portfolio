...
def test_basic_functionality():
    ...
    with app.app_context():
        ...
        # 测试查询
        found_record = BackupRecordExternal.query.filter_by(
            backup_id="test_backup_001"
        ).first()
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        try:
            ...
            with app.app_context():
                ...
                # 验证特定记录
                backup_001 = BackupRecordExternal.query.filter_by(
                    backup_id="backup_001"
                ).first()
                ...
                backup_002 = BackupRecordExternal.query.filter_by(
                    backup_id="backup_002"
                ).first()
                ...
                print(
                    f"backup_002状态: {backup_002.status}, 同步状态: {backup_002.sync_status}"
                )
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        try:
            ...
            with app.app_context():
                ...
                # 验证解决结果
                # 记录1应该被修复为completed
                resolved_record_1 = BackupRecordExternal.query.filter_by(
                    backup_id="conflict_001"
                ).first()
                ...
                # 记录2应该被修复为failed
                resolved_record_2 = BackupRecordExternal.query.filter_by(
                    backup_id="conflict_002"
                ).first()
                ...
                # 验证同步日志
                sync_logs = SyncLogExternal.query.filter_by(
...
def test_statistics():
    ...
    with app.app_context():
        ...
        logs = [
            SyncLogExternal(
                operation="sync_backup", record_type="backup", record_id="sync_001"
            ),
            SyncLogExternal(
