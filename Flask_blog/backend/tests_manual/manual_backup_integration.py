...
def test_backup_creation_flow():
    ...
    try:
        ...
        with app.app_context():
            ...
            from app.models import SHANGHAI_TZ, BackupRecord
            ...
            try:
                ...
                print(
                    f"      [OK] 外部元数据记录已创建，状态: {external_record.status}"
                )
            ...
            if not mysql_record or mysql_record.status != "completed":
                print(
                    f"   [ERROR] MySQL记录状态异常: {mysql_record.status if mysql_record else 'None'}"
                )
                return False
            ...
            if not external_record or external_record.status != "completed":
                print(
                    f"   [ERROR] 外部元数据记录状态异常: {external_record.status if external_record else 'None'}"
                )
                return False
            ...
            print(
                f"      文件大小: {mysql_record.file_size} / {external_record.file_size}"
            )
            ...
            if (
                ...
                and mysql_record.status == "running"
            ):
