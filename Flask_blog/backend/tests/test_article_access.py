def register_and_login(client, email, password="pass123", role=None):
    ...
    assert login.status_code == 200 and payload.get(
        "data"
    ), f"login failed: {login.status_code} {payload}"
...
def create_article_api(client, token, status="draft"):
    r = client.post(
        "/api/v1/articles/",
        json={"title": f"T-{status}", "content_md": "x"},
        headers={"Authorization": "Bearer " + token},
    )
    payload = r.get_json() or {}
    assert r.status_code == 201 and payload.get(
        "data"
    ), f"create article failed: {r.status_code} {payload}"
    aid = payload["data"]["id"]
    return aid
...
def _get_slug(client, token, article_id):
    r = client.get(
        f"/api/v1/articles/{article_id}", headers={"Authorization": "Bearer " + token}
    )
    payload = r.get_json() or {}
...
def test_slug_anonymous_and_published_visibility(client):
    ...
    # publish first draft
    client.post(
        f"/api/v1/articles/{draft_pub_id}/submit",
        headers={"Authorization": "Bearer " + token_author},
    )
    client.post(
        f"/api/v1/articles/{draft_pub_id}/approve",
...
def test_slug_other_author_cannot_view_draft(client):
    ...
    r = client.get(
        f"/api/v1/articles/slug/{slug}", headers={"Authorization": "Bearer " + token_a2}
    )
...
def test_anonymous_only_sees_published(client, app):
    ...
    submit = client.post(
        f"/api/v1/articles/{draft_id}/submit",
        headers={"Authorization": "Bearer " + token_author},
    )
    submit_payload = submit.get_json()
    assert (
        submit.status_code == 200
    ), f"submit failed: {submit.status_code} {submit_payload}"
    approve = client.post(
        f"/api/v1/articles/{draft_id}/approve",
        headers={"Authorization": "Bearer " + token_editor},
    )
    approve_payload = approve.get_json()
    assert (
        approve.status_code == 200
    ), f"approve failed: {approve.status_code} {approve_payload}"
...
def test_author_can_view_own_draft(app, client):
    ...
    r = client.get(
        f"/api/v1/articles/{draft_id}", headers={"Authorization": "Bearer " + token}
    )
    assert r.status_code == 200
...
def test_other_author_cannot_view_draft(app, client):
    ...
    r = client.get(
        f"/api/v1/articles/{draft_id}", headers={"Authorization": "Bearer " + token_a2}
    )
    assert r.status_code == 404
