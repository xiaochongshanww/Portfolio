...
def test_refresh_revokes_old_token(client, monkeypatch):
    ...
    # 注册+登录
    client.post(
        "/api/v1/auth/register", json={"email": "rr@test.com", "password": "pass123"}
    )
    login_resp = client.post(
        "/api/v1/auth/login", json={"email": "rr@test.com", "password": "pass123"}
    )
    assert login_resp.status_code == 200
    _capture_xsrf(client, login_resp)
    # 取得初始 refresh cookie
...
def test_logout_revokes_refresh(client, monkeypatch):
    ...
    client.post(
        "/api/v1/auth/register", json={"email": "lo@test.com", "password": "pass123"}
    )
    login_resp = client.post(
        "/api/v1/auth/login", json={"email": "lo@test.com", "password": "pass123"}
    )
    assert login_resp.status_code == 200
    _capture_xsrf(client, login_resp)
