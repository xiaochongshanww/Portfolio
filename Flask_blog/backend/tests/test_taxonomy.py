...
class TestCategories:
    ...
    def test_create_category_forbidden_author(self, client):
        ...
        resp = client.post(
            "/api/v1/taxonomy/categories/", json={"name": "Tech"}, headers=h
        )
    ...
    def test_update_category(self, client, app):
        ...
        h = auth_header(client, role="admin")
        resp = client.patch(
            f"/api/v1/taxonomy/categories/{c.id}",
            json={
                "name": "Updated",
            },
            headers=h,
...
class TestTags:
    ...
    def test_delete_tag_in_use(self, client, app):
        ...
        db.session.commit()
        db.session.execute(
            ArticleTag.__table__.insert().values(article_id=article.id, tag_id=tag.id)
        )
        db.session.commit()
        h = auth_header(client, role="admin")
