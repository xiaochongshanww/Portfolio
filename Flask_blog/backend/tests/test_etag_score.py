def test_search_etag_and_score(client, monkeypatch):
            ...
            return {
                ...
                "hits": [
                    ...
                    },
                    {
                        ...
                        "likes_count": 0,
                        "_rankingScore": 5.6,
                        "_formatted": {"title": "World", "content": "World"},
                    },
                ],
            }

    monkeypatch.setattr(
        "app.search.client.ensure_index", lambda: DummyIdx(), raising=False
    )
    monkeypatch.setattr(
        "app.search.indexer.ensure_index", lambda: DummyIdx(), raising=False
    )
    monkeypatch.setattr(
        "app.search.routes.ensure_index", lambda: DummyIdx(), raising=False
    )
    ...
    r2 = client.get(
        "/api/v1/search/", query_string={"q": "Hello"}, headers={"If-None-Match": etag}
    )
...
def test_public_article_list_etag(client):
    ...
    client.post(
        "/api/v1/auth/register", json={"email": "pa@test.com", "password": "pass123"}
    )
    login = client.post(
        "/api/v1/auth/login", json={"email": "pa@test.com", "password": "pass123"}
    )
    token = login.get_json()["data"]["access_token"]
    create = client.post(
        "/api/v1/articles/",
        json={"title": "公开文章", "content_md": "正文"},
...
def test_internal_articles_list_etag(client):
    ...
    client.post(
        "/api/v1/auth/register", json={"email": "in@test.com", "password": "pass123"}
    )
    login = client.post(
        "/api/v1/auth/login", json={"email": "in@test.com", "password": "pass123"}
    )
    token = login.get_json()["data"]["access_token"]
    client.post(
        "/api/v1/articles/",
        json={"title": "内部文章", "content_md": "正文"},
        headers={"Authorization": "Bearer " + token},
    )
...
def test_admin_users_list_etag(client):
    ...
    client.post(
        "/api/v1/auth/register", json={"email": "admin@test.com", "password": "pass123"}
    )
    login = client.post(
        "/api/v1/auth/login", json={"email": "admin@test.com", "password": "pass123"}
    )
    from app import db
...
def test_public_author_profile_etag(client):
    ...
    client.post(
        ...
        json={"email": "authorp@test.com", "password": "pass123"},
    )
    login = client.post(
        "/api/v1/auth/login", json={"email": "authorp@test.com", "password": "pass123"}
    )
    token = login.get_json()["data"]["access_token"]
    create = client.post(
        "/api/v1/articles/",
        json={"title": "A1", "content_md": "正文"},
        headers={"Authorization": "Bearer " + token},
    )
    art_id = create.get_json()["data"]["id"]
    from app import db
