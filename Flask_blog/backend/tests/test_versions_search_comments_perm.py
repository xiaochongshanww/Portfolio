...
def create_article(client, token, title="A1"):
    r = client.post(
        "/api/v1/articles/",
        json={"title": title, "content_md": "Hello"},
...
def publish_article(client, aid, author_token, editor_token):
    client.post(
        f"/api/v1/articles/{aid}/submit",
        headers={"Authorization": "Bearer " + author_token},
    )
    client.post(
        f"/api/v1/articles/{aid}/approve",
...
def test_article_versions_flow(client):
    ...
    login_token(client, "v_editor@test.com", role="editor")
    ...
    for content in ["First body", "Second body"]:
        ...
        # 显式再创建一个快照 (n+1)
        client.post(
            f"/api/v1/articles/{aid}/versions",
            headers={"Authorization": "Bearer " + author},
        )
    # 列出版本
    lst = client.get(
    ...
    # 回滚到版本1
    rb = client.post(
        f"/api/v1/articles/{aid}/versions/1/rollback",
        headers={"Authorization": "Bearer " + author},
    )
    assert rb.status_code == 200
    # 回滚生成新版本号 (5)
    get_v5 = client.get(
        f"/api/v1/articles/{aid}/versions/5",
        headers={"Authorization": "Bearer " + author},
    )
...
def test_search_fallback_and_tag_filter(client, app):
    ...
    for i, tags in enumerate((["x"], ["x", "y"], ["y"])):
        r = client.post(
            "/api/v1/articles/",
            json={"title": f"SE{i}", "content_md": "c", "tags": tags},
            headers={"Authorization": "Bearer " + author},
        )
...
def test_comment_visibility_permissions(client):
    ...
    login_token(client, "c_auth2@test.com")
    ...
    for content in ("P1", "P2"):
        client.post(
            "/api/v1/comments/",
            json={"article_id": aid, "content": "PENDING:" + content},
            headers={"Authorization": "Bearer " + author1},
        )
    ...
    tree_author_all = client.get(
        f"/api/v1/comments/article/{aid}/tree?include=all",
        headers={"Authorization": "Bearer " + author1},
    ).get_json()["data"]
    assert len(tree_author_all) == 2
    # 编辑也能看到 pending
    tree_editor_all = client.get(
        f"/api/v1/comments/article/{aid}/tree?include=all",
    ...
    client.post(
        f"/api/v1/comments/moderate/{first_id}",
        json={"action": "approve"},
        headers={"Authorization": "Bearer " + editor},
    )
    # 公共再次获取应只出现已审核 1 条
