...
def test_search_pagination_has_next(client, monkeypatch):
    ...
    monkeypatch.setattr(
        "app.search.client.ensure_index", lambda: DummyIdx2(), raising=False
    )
    monkeypatch.setattr(
        "app.search.indexer.ensure_index", lambda: DummyIdx2(), raising=False
    )

    r = client.get(
        "/api/v1/search/", query_string={"q": "Title", "page_size": 10, "page": 2}
    )
    assert r.status_code == 200
    data = r.get_json()["data"]
    assert data["page"] == 2
    ...
    r2 = client.get(
        "/api/v1/search/", query_string={"q": "Title", "page_size": 10, "page": 3}
    )
...
def test_search_multitag_and_or(client, monkeypatch):
    ...
    class DummyIdx3:
        def search(self, q, params=None):
            ...
            # crude simulation: if filters contain two separate tag clauses treat as AND => only doc 1  # noqa: E501
            tag_clause_count = sum(
                1 for f in filters if isinstance(f, str) and f.startswith("tags = '")
            )
    ...
    monkeypatch.setattr(
        "app.search.client.ensure_index", lambda: DummyIdx3(), raising=False
    )
    monkeypatch.setattr(
        "app.search.indexer.ensure_index", lambda: DummyIdx3(), raising=False
    )

    # OR mode
    r_or = client.get(
        "/api/v1/search/",
        query_string={"q": "x", "tags": "go,python", "match_mode": "or"},
    )
    ...
    r_and = client.get(
        "/api/v1/search/",
        query_string={"q": "x", "tags": "go,python", "match_mode": "and"},
    )
...
def test_version_diff_flow(client):
    # register & login
    client.post(
        "/api/v1/auth/register", json={"email": "vd@test.com", "password": "pass123"}
    )
    login = client.post(
        "/api/v1/auth/login", json={"email": "vd@test.com", "password": "pass123"}
    )
    ...
    create = client.post(
        "/api/v1/articles/",
        json={"title": "Diff文章", "content_md": "第一版内容"},
    ...
    # snapshot v1
    v1 = client.post(
        f"/api/v1/articles/{art_id}/versions",
        headers={"Authorization": "Bearer " + token},
    )
    ...
    client.put(
        f"/api/v1/articles/{art_id}",
        json={"content_md": "第二版内容\n新增一行"},
        headers={"Authorization": "Bearer " + token},
    )
    v2 = client.post(
        f"/api/v1/articles/{art_id}/versions",
    ...
    diff = client.get(
        f"/api/v1/articles/{art_id}/versions/1/diff",
    ...
    # invalid target
    diff_invalid = client.get(
        f"/api/v1/articles/{art_id}/versions/1/diff",
        query_string={"target": 99},
        headers={"Authorization": "Bearer " + token},
    )
