...
def public_articles():
    ...
    cache_key = (
        f"public:articles:list:{page}:{size}:{category_id or 'all'}:{tag or 'all'}"
    )
    if redis_client:
...
def public_article_detail(slug_or_id):
    ...
    if slug_or_id.isdigit():
        a = Article.query.filter_by(
            id=int(slug_or_id), status="published", deleted=False
        ).first()
    if not a:
        a = Article.query.filter_by(
            slug=slug_or_id, status="published", deleted=False
        ).first()
    if not a:
...
def public_taxonomy():
    ...
    if len(categories_with_count) > 0:
        print(
            f"[DEBUG] 前3个分类: {[(c.id, c.name, c.slug, c.article_count) for c in categories_with_count[:3]]}"  # noqa: E501
        )
    ...
    print(
        f'[DEBUG] 最终数据结构: categories={len(data["categories"])}, tags={len(data["tags"])}'  # noqa: E501
    )
