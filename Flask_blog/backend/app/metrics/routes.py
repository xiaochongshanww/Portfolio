...
def get_summary_stats():
    ...
    try:
        ...
        try:
            articles_total = (
                db.session.query(Article.id).filter_by(deleted=False).count()
            )
            articles_published = (
                db.session.query(Article.id)
                .filter_by(deleted=False, status="published")
                .count()
            )
            articles_draft = (
                db.session.query(Article.id)
                .filter_by(deleted=False, status="draft")
                .count()
            )
            articles_pending = (
                db.session.query(Article.id)
                .filter_by(deleted=False, status="pending")
                .count()
            )
            ...
            logger.info(
                f"文章数据: total={articles_total}, published={articles_published}"
            )
        except Exception as e:
        ...
        try:
            ...
            comments_pending = (
                db.session.query(Comment.id).filter_by(status="pending").count()
            )
            comments_approved = (
                db.session.query(Comment.id).filter_by(status="approved").count()
            )
        ...
        try:
            ...
            logger.info(
                f"分类标签数据: tags={tags_count}, categories={categories_count}"
            )
        except Exception as e:
...
def track_visit():
    ...
    try:
        ...
        from ..models import SHANGHAI_TZ, VisitorStats
        ...
        visitor = VisitorStats.query.filter_by(
            ip_address=ip_address, user_agent_hash=user_agent_hash, visited_date=today
        ).first()
        ...
        if visitor:
            logger.info(
                f"Found existing visitor, updating page views from {visitor.page_views}"
            )
        ...
        return jsonify(
            {
                "code": 0,
                "message": "ok",
                "data": {"tracked": True, "is_new_visitor": is_new_visitor},
            }
        )
    except Exception as e:
