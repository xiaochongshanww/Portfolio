        ...
        def decorated_function(*args, **kwargs):
            ...
            try:
                ...
                if error_occurred:
                    ...
                    # 根据响应状态码确定日志级别
                    status_code = (
                        getattr(response, "status_code", 200) if response else 200
                    )
                    ...
                    create_log_entry(
                        ...
                        source=LogSource.USER_ACTION,
                        message=(
                            f"{action_type}: {request.endpoint}"
                            if request
                            else f"{action_type}"
                        ),
                        request_id=request_id,
        ...
        def decorated_function(*args, **kwargs):
            ...
            try:
                ...
                create_log_entry(
                    level=LogLevel.ERROR,
                    source=LogSource.API_REQUEST,
                    message=f"API Request failed: {request.method} {request.endpoint} - {str(e)}",  # noqa: E501
                    request_id=request_id,
                    status_code=500,
                    duration_ms=duration_ms,
                    extra_data={"error": str(e)[:500], "url": request.url},
                )
                raise
...
def log_security_event(
    ...
    create_log_entry(
        ...
        extra_data={"event_type": event_type, "details": details or {}},
    )
