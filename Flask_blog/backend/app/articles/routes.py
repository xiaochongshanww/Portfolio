...
from ..models import Article, ArticleVersion, AuditLog
    ...
    if response.status_code == 200 and request.path.startswith("/articles/public/"):
        if "Cache-Control" not in response.headers:
            response.headers["Cache-Control"] = (
                f"public, max-age={current_app.config['PUBLIC_CACHE_MAX_AGE']}"
            )
    return response
...
def create_article_route():
    try:
        ...
        try:
            ...
            return (
                jsonify({"code": 4001, "message": "validation error", "data": str(ve)}),
                400,
            )

        article = create_article(parsed, request.user_id)
        focal_meta = _make_focal_crops(
            article.featured_image, article.featured_focal_x, article.featured_focal_y
        )
        payload = serialize_article(
            article, detail=True, include_user_flags=True, user_id=request.user_id
        )
        if focal_meta:
        ...
        return (
            jsonify({"code": e.code, "message": e.message, "data": e.data}),
            e.http_status,
        )
    except Exception as e:
        current_app.logger.exception("create_article failed")
        return (
            jsonify({"code": 5000, "message": "internal_error", "detail": str(e)}),
            500,
        )
...
def get_article(article_id):
    ...
    data = serialize_article(
        article, detail=True, include_user_flags=True, user_id=user_id
    )
    if redis_client and article.status == "published":
        try:
            cache_track_set(
                cache_key,
                current_app.config["CACHE_ARTICLE_DETAIL_TTL"],
                json.dumps(data),
            )
        except Exception:
            pass
...
def list_articles():
    ...
    cache_key = (
        f"articles:list:{page}:{size}:{status or '_'}:{tag or '_'}:{category or '_'}"
    )
    if redis_client:
        cached = redis_client.get(cache_key)
        if cached:
    ...
    if redis_client:
        try:
            cache_track_set(
                cache_key,
                current_app.config["CACHE_ARTICLE_LIST_TTL"],
                json.dumps(payload),
            )
        except Exception:
            pass
...
def like_toggle_route(article_id):
    try:
        ...
        return jsonify(
            {
                "code": 0,
                "data": {"action": action, "likes_count": count},
                "message": "ok",
            }
        )
    except BusinessError as e:
...
def bookmark_toggle_route(article_id):
    try:
        ...
        return jsonify(
            {
                "code": 0,
                "data": {"action": action, "bookmarks_count": count},
                "message": "ok",
            }
        )
    except BusinessError as e:
...
def list_bookmarks():
    ...
    page = int(request.args.get("page", 1))
    size = min(int(request.args.get("page_size", 10)), 50)
    q = Article.query.join(
        ArticleBookmark, Article.id == ArticleBookmark.article_id
    ).filter(ArticleBookmark.user_id == request.user_id, Article.deleted.is_(False))
    total = q.count()
    items = (
        q.order_by(ArticleBookmark.created_at.desc())
        .offset((page - 1) * size)
        .limit(size)
        .all()
    )
...
def list_versions(article_id):
    article = Article.query.get_or_404(article_id)
    if article.author_id != request.user_id and request.user_role not in (
        "editor",
        "admin",
    ):
    ...
    versions = (
        ArticleVersion.query.filter_by(article_id=article_id)
...
def get_version(article_id, version_no):
    ...
    if article.author_id != request.user_id and request.user_role not in (
        ...
        return jsonify({"code": 4030, "message": "forbidden"}), 403
    v = ArticleVersion.query.filter_by(
...
@articles_bp.route(
    "/<int:article_id>/versions/<int:version_no>/rollback", methods=["POST"]
)
@require_auth
def rollback_version_route(article_id, version_no):
    ...
    try:
        ...
        return jsonify(
            {
                "code": 0,
                "data": {"rolled_back_to": target_no, "new_version_no": new_no},
                "message": "ok",
            }
        )
    except BusinessError as e:
...
def diff_version_route(article_id, version_no):
    ...
    return jsonify(
        {
            "code": 0,
            "data": {"from": version_no, "to": target_no, "diff": diff},
            "message": "ok",
        }
    )
...
@workflow_transition(
    lambda article_id: Article.query.get(article_id), target_status="published"
)
...
@workflow_transition(
    lambda article_id: Article.query.get(article_id), target_status="draft"
)
...
@workflow_transition(
    lambda article_id: Article.query.get(article_id), target_status="pending"
)
...
@permission_required("workflow:submit")
@workflow_transition(
    lambda article_id: Article.query.get(article_id), target_status="scheduled"
)
                ...
                jsonify(
                    {
                        "code": 4001,
                        "message": "validation_error",
                        "data": {"field": "scheduled_at", "reason": "invalid datetime"},
                    }
                ),
                400,
            )
...
def delete_article_route(article_id):
    ...
    try:
        ...
        return jsonify(
            {"code": 0, "data": {"id": article.id, "deleted": True}, "message": "ok"}
        )
    except BusinessError as e:
        return jsonify({"code": e.code, "message": e.message}), e.http_status
...
def list_audit_logs(article_id):
    ...
    logs = (
        AuditLog.query.filter_by(article_id=article_id)
...
def public_list_articles():
    ...
    cache_key = f"public:articles:list:{page}:{size}:{tag or '_'}:{category_id or '_'}:{author_id or '_'}:{sort_field}:{'d' if desc else 'a'}"  # noqa: E501
    if redis_client:
        cached = redis_client.get(cache_key)
        if cached:
            payload = json.loads(cached)
    ...
    total = q.count()
    order_col = (
        Article.published_at if sort_field == "published_at" else Article.created_at
    )
    ...
    if redis_client:
        try:
            cache_track_set(
                cache_key,
                current_app.config["CACHE_PUBLIC_LIST_TTL"],
...
def public_article_by_slug(slug):
    ...
    try:
        if redis_client:
            ...
            if redis_client.set(vkey, "1", ex=3600, nx=True):
                Article.query.filter_by(id=article.id).update(
    ...
    if redis_client and not user_id:
        try:
            cache_track_set(
                cache_key,
                current_app.config["CACHE_PUBLIC_ARTICLE_TTL"],
                json.dumps(data),
            )
        except Exception:
            pass
