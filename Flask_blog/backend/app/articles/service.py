...
def _batch_likes_bookmarks(article_ids: list[int]):
    ...
    if not article_ids:
        return {}, {}
    likes_q = (
        db.session.query(ArticleLike.article_id, func.count())
        .filter(ArticleLike.article_id.in_(article_ids))
        .group_by(ArticleLike.article_id)
        .all()
    )
    bookmarks_q = (
        db.session.query(ArticleBookmark.article_id, func.count())
        .filter(ArticleBookmark.article_id.in_(article_ids))
        .group_by(ArticleBookmark.article_id)
        .all()
    )
...
def serialize_article(
    ...
    if include_user_flags:
        if user_id:
            liked = (
                ArticleLike.query.filter_by(article_id=a.id, user_id=user_id).first()
                is not None
            )
            bookmarked = (
                ArticleBookmark.query.filter_by(
                    article_id=a.id, user_id=user_id
                ).first()
                is not None
            )
            data["liked"] = liked
...
def _generate_unique_slug(
    initial_slug: str, exclude_id: int | None = None, max_tries: int = 50
) -> str:
...
def _make_focal_crops(featured_image, focal_x, focal_y):
    if not featured_image or focal_x is None or focal_y is None:
        return {}
    try:
        return generate_focal_crops(
            featured_image, focal_x, focal_y, current_app.config["UPLOAD_DIR"]
        )
    except Exception:
        return {}
...
def log_action(article_id, operator_id, action, note=None):
    try:
        al = AuditLog(
            article_id=article_id, operator_id=operator_id, action=action, note=note
        )
        db.session.add(al)
        db.session.commit()
    except Exception:
...
def rollback_to_version(article: Article, target_version_no: int, user_id: int):
    target = ArticleVersion.query.filter_by(
        article_id=article.id, version_no=target_version_no
    ).first()
    if not target:
...
def diff_versions(article_id: int, version_no_a: int, version_no_b: int):
    v1 = ArticleVersion.query.filter_by(
        article_id=article_id, version_no=version_no_a
    ).first()
    v2 = ArticleVersion.query.filter_by(
        article_id=article_id, version_no=version_no_b
    ).first()
    if not v1 or not v2:
        raise BusinessError(4040, "version_not_found", 404)
    ...
    diff = list(
        difflib.unified_diff(
            text1,
            text2,
            fromfile=f"v{version_no_a}",
            tofile=f"v{version_no_b}",
            lineterm="",
        )
    )
...
def schedule_article(
    article: Article, scheduled_at_dt: datetime, user_id: int, user_role: str
):
    if article.author_id != user_id and user_role not in ("editor", "admin"):
        raise BusinessError(2002, "forbidden", 403)
    if article.deleted:
...
def toggle_bookmark(article_id: int, user_id: int) -> tuple[str, int]:
    ...
    if article.deleted or article.status != "published":
        raise BusinessError(4040, "not_found", 404)
    existing = ArticleBookmark.query.filter_by(
        article_id=article_id, user_id=user_id
    ).first()
    if existing:
        db.session.delete(existing)
