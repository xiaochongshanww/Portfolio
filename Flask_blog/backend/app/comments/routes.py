...
def list_comments_tree(article_id):
    ...
    q = Comment.query.filter_by(article_id=article_id).order_by(
...
def moderate(comment_id):
    ...
    db.session.commit()
    return jsonify(
        {"code": 0, "data": {"id": c.id, "status": c.status}, "message": "ok"}
    )
...
def moderate_batch():
    ...
    return jsonify(
        {
            "code": 0,
            "data": {"updated_count": len(comments), "status": new_status},
            "message": "ok",
        }
    )
