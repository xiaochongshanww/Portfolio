...
def me():
    ...
    return jsonify(
        {"code": 0, "message": "ok", "data": serialize_user(u, include_email=True)}
    )
...
def update_me():
    ...
    try:
        ...
        return (
            jsonify({"code": 4001, "message": "invalid request data", "data": str(e)}),
            400,
        )
    ...
    if hasattr(parsed, "social_links") and parsed.social_links is not None:
        try:
            ...
            if not isinstance(parsed.social_links, dict):
                return (
                    jsonify(
                        {"code": 4001, "message": "social_links must be a dictionary"}
                    ),
                    400,
                )
...
def change_role(user_id):
    ...
    audit_log(
        ...
        f"用户 {user_id} 角色变更: {old} → {u.role}",
    )
    return jsonify(
        {
            "code": 0,
            "message": "ok",
            "data": {"id": u.id, "old_role": old, "new_role": u.role},
        }
    )
    ...
    if redis_client:
        ...
        if cached:
            try:
                ...
                if CACHE_HIT_TOTAL:
                    try:
                        CACHE_HIT_TOTAL.labels("public_author_profile").inc()
                    except Exception:
                        pass
            ...
            if CACHE_MISS_TOTAL:
                try:
                    CACHE_MISS_TOTAL.labels("public_author_profile").inc()
                except Exception:
                    pass
    u = User.query.get_or_404(user_id)
...
def public_author_articles(user_id):
    ...
    if METRICS_ENABLED and PUBLIC_AUTHOR_ARTICLES_REQUESTS_TOTAL:
        try:
            ...
            pass
    cache_key = f"public:author_articles:{user_id}:{page}:{size}:{sort_field}:{'d' if desc else 'a'}"  # noqa: E501
    if redis_client:
        cached = redis_client.get(cache_key)
        if cached:
            try:
                ...
                if CACHE_HIT_TOTAL:
                    try:
                        CACHE_HIT_TOTAL.labels("public_author_articles").inc()
                    except Exception:
                        pass
                etag = compute_etag(payload)
                if request.headers.get("If-None-Match") == etag:
            ...
            if CACHE_MISS_TOTAL:
                try:
                    CACHE_MISS_TOTAL.labels("public_author_articles").inc()
                except Exception:
                    pass
