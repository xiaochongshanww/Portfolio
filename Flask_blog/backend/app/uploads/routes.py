...
# 新增: 统一尺寸配置
VARIANTS = [("lg", 1600), ("md", 800), ("sm", 400), ("thumb", 200)]
...
def upload_image():
    ...
    try:
        ...
        for label, max_w in VARIANTS:
            if width <= max_w:
                ...
                variants_meta.append(
                    {
                        "label": label,
                        "url": f"/uploads/{subdir}/{orig_name}",
                        "width": width,
                        "height": height,
                    }
                )
            ...
            variants_meta.append(
                {
                    "label": label,
                    "url": f"/uploads/{subdir}/{variant_name}",
                    "width": max_w,
                    "height": new_h,
                }
            )
        ...
        try:
            ...
            lqip_b64 = "data:image/jpeg;base64," + base64.b64encode(
                buf.getvalue()
            ).decode("utf-8")
        except Exception:
            lqip_b64 = None
    ...
    srcset = (
        ", ".join(f"{v['url']} {v['width']}w" for v in variants_meta if v.get("width"))
        if variants_meta
        else None
    )
    data = {
        ...
        "lqip": lqip_b64,
        "webp": (
            f"/uploads/{subdir}/{name_root}.webp"
