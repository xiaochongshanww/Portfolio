...
PATHS = {
            ...
            "responses": {
                "200": {
                    ...
                    "content": {
                        "application/json": {
                            "schema": {
                                "$ref": "#/components/schemas/ErrorCodeCatalogResponse"
                            }
                        }
                    },
                }
        ...
        "get": {
            ...
            "parameters": [
                ...
                {"name": "author_id", "in": "query", "schema": {"type": "integer"}},
                {
                    ...
                    "schema": {
                        "type": "string",
                        "description": "published_at:desc|published_at:asc|created_at:desc|created_at:asc|likes_count:desc|likes_count:asc|views_count:desc|views_count:asc",  # noqa: E501
                    },
                },
            ],
            "responses": {
    ...
    "/api/v1/articles/public/slug/{slug}": {
        "get": {
            ...
            "description": "支持 ETag / If-None-Match；views_count 为近实时去重浏览次数(同用户或 IP+UA 1 小时内只计 1 次)",  # noqa: E501
            "parameters": [
                {
                    "name": "slug",
                    "in": "path",
    ...
    "/api/v1/articles/public/hot": {
        "get": {
            ...
            "description": "基于窗口(默认48h)的去重浏览+点赞+时间衰减组合得分。score = log10(views+1)*0.7 + likes*0.5 + 1/(1+hours/24)。支持 page,page_size,window_hours。支持 ETag / If-None-Match",  # noqa: E501
            "parameters": [
                {
                    "name": "page",
                    "in": "query",
                ...
                },
                {
                    ...
                    "description": "统计窗口(小时)，默认48，最大168(7天)",
                },
            ],
            "responses": {
                "200": {
    ...
    "/api/v1/search/": {
        "get": {
            ...
            "description": "全文搜索文章；支持 ETag；ranking rules 纳入 likes_count, views_count, published_at",  # noqa: E501
            "parameters": [
                {
                    "name": "q",
                    "in": "query",
                ...
                {"name": "tags", "in": "query", "schema": {"type": "string"}},
                {
                    "name": "match_mode",
                    "in": "query",
                    "schema": {"type": "string", "enum": ["any", "all", "phrase"]},
                },
                {"name": "category_id", "in": "query", "schema": {"type": "integer"}},
                {"name": "author_id", "in": "query", "schema": {"type": "integer"}},
                {
                    ...
                    "schema": {
                        "type": "string",
                        "description": "published_at:desc|published_at:asc|created_at:desc|created_at:asc|likes_count:desc|likes_count:asc|views_count:desc|views_count:asc|_score:desc",  # noqa: E501
                    },
                },
            ],
            "responses": {
            ...
            "responses": {
                "200": {
                    ...
                    "content": {
                        "application/json": {
                            "schema": {
                                "$ref": "#/components/schemas/ArticleVersionListResponse"  # noqa: E501
                            }
                        }
                    },
                },
            ...
            "responses": {
                "200": {
                    ...
                    "content": {
                        "application/json": {
                            "schema": {
                                "$ref": "#/components/schemas/CommentPendingListResponse"  # noqa: E501
                            }
                        }
                    },
                },
...
SCHEMAS = {
    ...
    "CreateVersionResponse": {
        ...
        "properties": {
            ...
            "data": {
                "type": "object",
                "properties": {"version_no": {"type": "integer"}},
            },
        },
    },
    ...
    "CommentTreeNode": {
        ...
        "properties": {
            ...
            "children": {
                "type": "array",
                "items": {"$ref": "#/components/schemas/CommentTreeNode"},
            },
        },
    },
    "CommentTreeResponse": {
    ...
    "ErrorCodeCatalogResponse": {
        ...
        "properties": {
            ...
            "data": {
                "type": "array",
                "items": {"$ref": "#/components/schemas/ErrorCodeItem"},
            },
        },
    },
    # Uploads
...
SCHEMAS.update(
    {
            ...
            "properties": {
                ...
                "data": {
                    ...
                    "properties": {
                        "action": {
                            "type": "string",
                            "description": "bookmarked 或 removed",
                        }
                    },
                },
            },
        ...
        "RejectRequest": {
            "type": "object",
            "properties": {"reason": {"type": "string", "description": "拒绝原因"}},
            "required": ["reason"],
        },
    }
)
    ...
    try:
        ...
        for s in ("ArticleCreate", "ArticleUpdate"):
            ...
            _upd(
                ...
                minLength=1,
                maxLength=150,
                description="URL slug，允许小写字母/数字/连字符",
            )
        ...
        try:
            ...
            filters_props = SCHEMAS["SearchResultResponse"]["properties"]["data"][
                "allOf"
            ][1]["properties"]["filters"]["properties"]
            if "status" in filters_props:
                filters_props["status"].setdefault("enum", status_prop["enum"])
            if "match_mode" in filters_props:
                filters_props["match_mode"].setdefault(
                    "enum", ["default", "all", "any", "exact"]
                )
            if "sort" in filters_props:
                filters_props["sort"].setdefault(
                    "enum",
                    [
                        "published_at_desc",
                        "published_at_asc",
        ...
        if "CommentModerate" in SCHEMAS:
            SCHEMAS["CommentModerate"]["properties"]["action"].setdefault(
                "enum", ["approve", "reject"]
            )
        if "BookmarkToggleResponse" in SCHEMAS:
            SCHEMAS["BookmarkToggleResponse"]["properties"]["data"]["properties"][
                "action"
            ].setdefault("enum", ["bookmarked", "removed"])
        if "LikeToggleResponse" in SCHEMAS:
            SCHEMAS["LikeToggleResponse"]["properties"]["data"]["properties"][
                "action"
            ].setdefault("enum", ["liked", "unliked"])
        ...
        ERROR_CODES = [
            ...
            {
                "code": 3001,
                "message": "workflow_invalid_state",
                "description": "当前状态不允许该操作",
            },
            {
                "code": 3002,
                "message": "workflow_transition_conflict",
                "description": "状态机并发冲突",
            },
