...
def search():
    ...
    if force_fallback:
        ...
        if sort_expr and (
            ...
            col = (
                Article.published_at if field == "published_at" else Article.created_at
            )
        ...
        if facets_wanted:
            ...
            if "category_id" in facets_wanted:
                rows = (
                    base_q.with_entities(A.category_id, func.count(A.id))
            ...
            if "author_id" in facets_wanted:
                rows = (
                    base_q.with_entities(A.author_id, func.count(A.id))
                    .group_by(A.author_id)
                    .all()
                )
            ...
            if "tags" in facets_wanted:
                ...
                # 统计发布文章中标签使用频次
                tag_rows = (
                    query.session.query(T.slug, func.count(T.slug))
                    .join(A.tags)
                    .filter(A.deleted.is_(False), A.status == "published")
                    .group_by(T.slug)
                    .all()
                )
