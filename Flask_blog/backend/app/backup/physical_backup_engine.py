...
class PhysicalBackupEngine:
    ...
    def _setup_logger(self) -> logging.Logger:
        ...
        formatter = logging.Formatter(
            "%(asctime)s [PHYSICAL] %(levelname)s - %(message)s"
        )
        ...
        if (
            not logger.handlers
            or len([h for h in logger.handlers if isinstance(h, logging.StreamHandler)])
            == 0
        ):
    ...
    def create_backup(self, backup_id: Optional[str] = None) -> Dict[str, Any]:
        ...
        try:
            ...
            metadata = {
                ...
                "compression_ratio": (
                    round(compressed_size / raw_backup_size, 3)
                    if compressed_size and raw_backup_size > 0
                    else None
                ),
                # 时间统计信息（秒）
                "timing": {
                ...
                "performance": {
                    "backup_speed_mbps": (
                        round((raw_backup_size / 1024 / 1024) / backup_duration, 2)
                        if backup_duration > 0
                        else 0
                    ),
                    "total_speed_mbps": (
                        round((raw_backup_size / 1024 / 1024) / total_duration, 2)
                        if total_duration > 0
                        else 0
                    ),
                },
            ...
            self.logger.info(
                f"备份大小: {raw_backup_size:,} bytes ({raw_backup_size/1024/1024:.1f} MB)"
            )
            if compressed_size:
                self.logger.info(
                    f"压缩后大小: {compressed_size:,} bytes ({compressed_size/1024/1024:.1f} MB)"  # noqa: E501
                )
                self.logger.info(f"压缩比: {metadata['compression_ratio']:.1%}")
            self.logger.info(
                f"总耗时: {total_duration:.2f}秒, 备份速度: {metadata['performance']['total_speed_mbps']:.1f} MB/s"  # noqa: E501
            )
            ...
            return {
                ...
                "summary": {
                    ...
                    "backup_size": (
                        compressed_size if compressed_size else raw_backup_size
                    ),
                    "backup_size_text": self._format_file_size(
                        compressed_size if compressed_size else raw_backup_size
                    ),
    ...
    def _get_container_volume(self) -> Optional[str]:
        ...
        try:
            ...
            if result.returncode == 0:
                ...
                for mount in mounts:
                    if (
                        mount.get("Destination") == "/var/lib/mysql"
    ...
    def _get_database_info(self) -> Dict[str, Any]:
        ...
        try:
            # 获取MySQL版本
            version_cmd = ["docker", "exec", self.container_name, "mysql", "--version"]
    ...
    def _hot_backup_via_docker(self, backup_dir: Path) -> Dict[str, Any]:
        ...
        try:
            ...
            backup_cmd = [
                ...
                "--rm",
                "-v",
                f"{self.volume_name}:/source:ro",  # 只读挂载源volume
                "-v",
                f"{backup_dir.absolute()}:/backup",  # 挂载备份目录
                "alpine:latest",
                "sh",
                "-c",
            ...
            result = subprocess.run(
                backup_cmd, capture_output=True, text=True, timeout=1800  # 30分钟超时
            )
        ...
        try:
            ...
            try:
                ...
                backup_result = subprocess.run(
                    backup_cmd, capture_output=True, text=True, timeout=300
                )
                ...
                unlock_result = subprocess.run(
                    unlock_cmd, capture_output=True, text=True
                )
    ...
    def get_storage_statistics(self) -> Dict[str, Any]:
        ...
        try:
            ...
            # 存储效率分析
            average_backup_size = (
                total_archive_size // total_backups if total_backups > 0 else 0
            )
            ...
            statistics = {
                ...
                "total_compressed_size_text": self._format_file_size(
                ...
                "compression_savings_text": self._format_file_size(
                    total_raw_size - total_compressed_size
                ),
                # 平均值
                "average_backup_size": average_backup_size,
                "average_backup_size_text": self._format_file_size(average_backup_size),
                "average_raw_size": (
                    total_raw_size // total_backups if total_backups > 0 else 0
                ),
                # 大小分布
                ...
                "storage_efficiency": {
                    "files_vs_raw_ratio": (
                        round(backup_root_size / total_raw_size, 3)
                        if total_raw_size > 0
                        else 0
                    ),
            ...
            self.logger.info(
                f"存储统计完成 - 总备份: {total_backups}, 总存储: {self._format_file_size(backup_root_size)}"  # noqa: E501
            )
            return statistics
        ...
        try:
            ...
            for item in self.backup_root.iterdir():
                ...
                if item.is_file():
                    ...
                    total_size += dir_size
                    self.logger.debug(
                        f"包含备份目录 {item.name}: {self._format_file_size(dir_size)}"
                    )
...
def main():
    ...
    if len(sys.argv) < 2:
        print(
            "用法: python physical_backup_engine.py <create|list|info|delete> [backup_id]"
        )
        sys.exit(1)
