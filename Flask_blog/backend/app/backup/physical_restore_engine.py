...
class PhysicalRestoreEngine:
    ...
    def _setup_logger(self) -> logging.Logger:
        ...
        formatter = logging.Formatter(
            "%(asctime)s [PHYSICAL_RESTORE] %(levelname)s - %(message)s"
        )
    ...
    def _validate_backup(self, backup_id: str) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            if original_volume_name:
                ...
                self.logger.warning("备份元数据中缺少 volume_name 信息，将使用自动检测")
                # 降级使用自动检测
    ...
    def _prepare_for_restore(self) -> Dict[str, Any]:
        ...
        try:
            ...
            if not stop_result["success"]:
                self.logger.warning(
                    f"停止MySQL容器警告: {stop_result.get('error', '')}"
                )
            ...
            if not backup_current_result["success"]:
                self.logger.warning(
                    f"当前数据备份警告: {backup_current_result.get('error', '')}"
                )
    ...
    def _backup_current_data(self) -> Dict[str, Any]:
        ...
        try:
            ...
            backup_cmd = [
                ...
                "-v",
                f"{current_backup_dir.absolute()}:/backup",
                "alpine:latest",
                "sh",
                "-c",
                f'cd /source && tar czf /backup/pre_restore_{int(time.time())}.tar.gz . || echo "No data to backup"',  # noqa: E501
            ]
    ...
    def _clear_mysql_volume(self) -> Dict[str, Any]:
        ...
        try:
            ...
            result = subprocess.run(
                clear_cmd, capture_output=True, text=True, timeout=120
            )
    ...
    def _restore_from_archive(
        self, archive_path: Path, backup_id: str
    ) -> Dict[str, Any]:
        """从压缩归档恢复"""
        try:
            ...
            try:
                ...
                if extract_result.returncode != 0:
                    return {
                        "success": False,
                        "error": f"解压归档失败: {extract_result.stderr}",
                    }
    ...
    def _restore_from_data_file(self, data_file: Path) -> Dict[str, Any]:
        ...
        try:
            ...
            restore_cmd = [
                ...
                "--rm",
                "-v",
                f"{data_file.absolute()}:/backup.tar.gz:ro",
                "-v",
                f"{self.volume_name}:/target",
                "alpine:latest",
                "sh",
                "-c",
                "cd /target && tar xzf /backup.tar.gz",
            ]
    ...
    def _validate_restore(self) -> Dict[str, Any]:
        ...
        try:
            ...
            try:
                ...
                validation_results["database_connection"] = (
            ...
            try:
                tables_cmd = [
                    ...
                    "blog",
                    "-e",
                    'SELECT COUNT(*) as table_count FROM information_schema.tables WHERE table_schema="blog";',  # noqa: E501
                ]
            ...
            try:
                ...
                integrity_result = subprocess.run(
                    integrity_cmd, capture_output=True, text=True, timeout=10
                )
            ...
            validation_results["overall_health"] = (
                validation_results.get("database_connection", False)
                and validation_results.get("tables_check", False)
                and validation_results.get("data_integrity_check", False)
            )
