...
class BackupRecordExternal(external_db.Model):
    ...
    backup_id = external_db.Column(
        external_db.String(255), nullable=False, unique=True, index=True
    )
    backup_type = external_db.Column(
        external_db.String(50), nullable=False, default="physical"
    )
    status = external_db.Column(
        external_db.String(50), nullable=False, default="pending", index=True
    )
    ...
    encryption_enabled = external_db.Column(
        external_db.Boolean, nullable=False, default=False
    )
    ...
    # 同步管理字段
    mysql_record_id = external_db.Column(
        external_db.Integer, nullable=True, index=True
    )  # 主数据库记录ID
    sync_status = external_db.Column(
        external_db.String(20), nullable=False, default="pending", index=True
    )  # pending, synced, conflict, verified
    ...
    def resolve_conflict_by_file_check(self) -> str:
        ...
        if file_exists:
            ...
            if self.backup_type == "physical" and self.status == "completed":
                ...
                self.update_sync_status(
                    "verified", f"状态从{old_status}修复为failed（文件不存在）"
                )
    ...
    def _resolve_status_conflict_intelligently(self) -> str:
        ...
        try:
            ...
            if external_status == "completed" and mysql_status == "running":
                ...
                self.update_sync_status(
                    "verified",
                    f"智能解决：以外部元数据为准，从{old_status}修复为completed（物理恢复后正常现象）",
                )
                return "fixed_to_completed"

            # 2. 如果MySQL状态是completed，而外部是pending/running，通常MySQL更准确
            elif mysql_status == "completed" and external_status in [
                "pending",
                "running",
            ]:
            ...
            # 2. 如果外部状态是completed，但MySQL是failed，需要文件验证
                ...
                if file_exists or self.backup_type == "physical":
                    ...
                    self.update_sync_status(
                        "verified", "智能解决：维持completed状态（文件存在或物理备份）"
                    )
                    return "verified_consistent"
                else:
                    ...
                    self.update_sync_status(
                        "verified",
                        f"智能解决：从{old_status}同步为{mysql_status}（文件不存在）",
                    )
                    return "fixed_to_failed"
    ...
    def __repr__(self):
        return f"<BackupRecordExternal {self.backup_id} status={self.status} sync={self.sync_status}>"  # noqa: E501
...
class RestoreRecordExternal(external_db.Model):
    ...
    restore_type = external_db.Column(
        external_db.String(50), nullable=False, default="full"
    )
    status = external_db.Column(
        external_db.String(50), nullable=False, default="pending", index=True
    )
    ...
    error_message = external_db.Column(external_db.Text, nullable=True)
    restore_options_json = external_db.Column(
        external_db.Text, nullable=True
    )  # JSON格式
    requested_by = external_db.Column(
        external_db.String(255), nullable=False, default="unknown"
    )
    restored_databases_count = external_db.Column(
        external_db.Integer, nullable=False, default=0
    )
    ...
    def __repr__(self):
        return f"<RestoreRecordExternal {self.restore_id} status={self.status} sync={self.sync_status}>"  # noqa: E501
...
class SyncLogExternal(external_db.Model):
    ...
    id = external_db.Column(external_db.Integer, primary_key=True)
    operation = external_db.Column(
        external_db.String(100), nullable=False, index=True
    )  # sync_backup, sync_restore, verify_files, resolve_conflict
    record_type = external_db.Column(
        external_db.String(50), nullable=False
    )  # backup, restore
    record_id = external_db.Column(
        external_db.String(255), nullable=False, index=True
    )  # backup_id or restore_id
    ...
    sync_direction = external_db.Column(
        external_db.String(50), nullable=False, default="mysql_to_sqlite"
    )  # mysql_to_sqlite, sqlite_to_mysql, bidirectional
    conflict_resolved = external_db.Column(
        external_db.Boolean, nullable=False, default=False
    )
...
class ExternalMetadataManager:
    ...
    def init_app(self, app):
        ...
        with app.app_context():
            try:
                ...
                app.logger.info(
                    f"外部元数据数据库验证成功: {stats['total_backup_records']} 条记录"
                )
            except Exception as e:
                ...
                try:
                    ...
                    if hasattr(mysql_record, "__dict__"):
                        record_dict = {
                            ...
                            "extra_data": (
                                mysql_record.get_extra_data()
                                if hasattr(mysql_record, "get_extra_data")
                                else None
                            ),
                        }
                    else:
                    ...
                    if not existing_record:
                        ...
                        self._log_sync_operation(
                            ...
                            details={
                                "action": "created",
                                "mysql_id": record_dict.get("id"),
                            },
                        )

                    else:
                        # 检查是否需要更新
                        sync_result = self._sync_existing_record(
                    ...
                    error_msg = f"同步备份记录 {mysql_record.get('backup_id', 'unknown')} 失败: {e}"  # noqa: E501
    ...
    def _sync_existing_record(
        self, existing_record: BackupRecordExternal, mysql_record: dict
    ) -> str:
        """同步现有记录"""
        ...
        if old_status != mysql_status:
            ...
            if self._is_status_conflict(old_status, mysql_status, file_exists):
                existing_record.update_sync_status(
                    "conflict",
                    f"状态冲突: 外部({old_status}) vs MySQL({mysql_status}), 文件存在: {file_exists}",  # noqa: E501
                )
    ...
    def _is_status_conflict(
        self, external_status: str, mysql_status: str, file_exists: bool
    ) -> bool:
        """判断是否为状态冲突"""
        # 常见冲突场景：
    ...
    def _update_record_from_mysql(
        self, external_record: BackupRecordExternal, mysql_record: dict
    ):
        """从MySQL记录更新外部记录"""
        external_record.status = mysql_record.get("status", external_record.status)
        external_record.file_path = mysql_record.get(
            "file_path", external_record.file_path
        )
        external_record.file_size = mysql_record.get(
            "file_size", external_record.file_size
        )
        external_record.compressed_size = mysql_record.get(
            "compressed_size", external_record.compressed_size
        )
        external_record.compression_ratio = mysql_record.get(
            "compression_ratio", external_record.compression_ratio
        )
        external_record.checksum = mysql_record.get(
            "checksum", external_record.checksum
        )
        external_record.encryption_enabled = mysql_record.get(
            "encryption_enabled", external_record.encryption_enabled
        )
        external_record.error_message = mysql_record.get(
        ...
        # 更新额外数据
        external_record.extra_data = mysql_record.get(
            "extra_data", external_record.extra_data
        )
        ...
        try:
            ...
            backup_stats = external_db.session.query(
                ...
                external_db.func.sum(
                    external_db.case(
                        (BackupRecordExternal.sync_status == "synced", 1), else_=0
                    )
                ).label("synced"),
                external_db.func.sum(
                    external_db.case(
                        (BackupRecordExternal.sync_status == "conflict", 1), else_=0
                    )
                ).label("conflicts"),
                external_db.func.sum(
                    external_db.case(
                        (BackupRecordExternal.sync_status == "pending", 1), else_=0
                    )
                ).label("pending"),
                external_db.func.sum(
                    external_db.case(
                        (BackupRecordExternal.sync_status == "verified", 1), else_=0
                    )
                ).label("verified"),
            ).first()
            ...
            return {
                ...
                "last_sync_time": (
                    last_sync_result.isoformat() if last_sync_result else None
                ),
                "file_verified_count": file_verified_count,
                "recent_operations_24h": {
                    op.operation: op.count for op in recent_operations
                },
                "database_path": self.db_path,
                "database_size_mb": self._get_database_size_mb(),
            }
        except Exception as e:
            return {"error": f"获取统计信息失败: {e}", "database_path": self.db_path}
    ...
    def create_backup_record(
        ...
        try:
            ...
            extra_data = {"description": description, "requested_by": requested_by}
    ...
    def update_backup_record(self, backup_id: str, **kwargs) -> bool:
        ...
        try:
            ...
            record = (
                session.query(BackupRecordExternal)
    ...
    def get_statistics(self) -> Dict[str, Any]:
        ...
        try:
            ...
            return {
                ...
                "success_rate": (
                    round(completed_backups / total_backups * 100, 2)
                    if total_backups > 0
                    else 0
                ),
                "database_path": self.db_path,
                ...
                if not external_record:
                    ...
                    try:
                        self.create_backup_record(
                            ...
                            backup_type=mysql_record.backup_type,
                            status=mysql_record.status,
                            description=mysql_record.get_extra_data().get(
                                "description", ""
                            ),
                            requested_by=mysql_record.get_extra_data().get(
                                "requested_by", ""
                            ),
                        )
                        synced_count += 1
                        ...
                        current_app.logger.warning(
                            f"同步记录 {mysql_record.backup_id} 失败: {e}"
                        )
                else:
                    # 检测状态冲突而不是直接覆盖
                    mysql_status = mysql_record.status
                    ...
                    if mysql_status != external_status:
                        ...
                        current_app.logger.info(
                            f"检测到状态冲突 {mysql_record.backup_id}: MySQL={mysql_status}, 外部={external_status}"  # noqa: E501
                        )
                        ...
                        try:
                            ...
                            # 重新获取记录以避免会话混淆
                            fresh_record = (
                                conflict_session.query(BackupRecordExternal)
                                .filter_by(backup_id=mysql_record.backup_id)
                                .first()
                            )
                            if fresh_record:
                            ...
                            current_app.logger.warning(
                                f"设置冲突状态失败 {mysql_record.backup_id}: {conflict_error}"
                            )
                            if conflict_session:
                                try:
    ...
    def sync_to_mysql_backup_records(self) -> int:
        ...
        try:
            ...
            for external_record in external_records:
                # 查找对应的MySQL记录
                mysql_record = BackupRecord.query.filter_by(
