...
class SQLAlchemyTableDiscovery:
    ...
    def get_detailed_table_info() -> Dict[str, TableInfo]:
        ...
        try:
            for table_name, table_obj in db.metadata.tables.items():
                ...
                # 分析依赖关系
                dependencies = SQLAlchemyTableDiscovery._get_table_dependencies(
                    table_obj
                )
    ...
    def _is_junction_table(table_obj) -> bool:
        ...
        if (
            ...
            and len(foreign_key_columns) >= len(columns) * 0.5
        ):
            return True
    ...
    @staticmethod
    def _assess_table_importance(
        table_name: str, table_obj, is_junction: bool
    ) -> TableImportance:
        """评估表的重要性"""
    ...
    def get_tables_from_migrations() -> Set[str]:
        ...
        try:
            ...
            for migration_file in migration_files:
                tables_in_migration = MigrationHistoryAnalyzer._parse_migration_file(
                    migration_file
                )
    ...
    def _parse_migration_file(migration_path: Path) -> Dict[str, Set[str]]:
        ...
        try:
            ...
            upgrade_match = re.search(
                r"def upgrade\(\):.*?(?=def downgrade\(\):|\Z)", content, re.DOTALL
            )
            if upgrade_match:
            ...
            if downgrade_match:
                downgrade_match.group(0)

            # 在 upgrade() 部分查找表创建操作
            create_patterns = [
                r"op\.create_table\(['\"](\w+)['\"]",  # op.create_table('table_name'
                r"op\.create_table\(\s*['\"](\w+)['\"]",  # op.create_table( 'table_name'  # noqa: E501
                r"op\.create_table\(\s*['\"](\w+)['\"]",  # with whitespace
                r"op\.create_table\(\s*[\r\n\s]*['\"](\w+)['\"]",  # multiline format
                r"CREATE TABLE ['\"]?(\w+)['\"]?",  # CREATE TABLE table_name
                r"CREATE TABLE IF NOT EXISTS ['\"]?(\w+)['\"]?",  # CREATE TABLE IF NOT EXISTS  # noqa: E501
            ]
...
class SmartTableValidator:
    ...
    def get_expected_tables_smart(self) -> Tuple[Set[str], Dict[str, TableInfo]]:
        ...
        try:
            ...
            if new_from_migrations:
                ...
                self.logger.info(
                    f"✅ 从迁移历史补充了 {len(new_from_migrations)} 张表: {new_from_migrations}"
                )
        except Exception as e:
        ...
        if not all_tables:
            try:
                ...
                self.logger.info(
                    f"⚠️ 使用兜底方案，从当前数据库获取了 {len(current_tables)} 张表"
                )
            except Exception as e:
    ...
    def validate_backup_completeness(self, sql_content: str) -> Dict[str, Any]:
        ...
        # Step 4: 分析缺失表的严重性
        severity_analysis = self._analyze_missing_severity(
            missing_tables, table_details
        )
    ...
    def _analyze_missing_severity(
        self, missing_tables: Set[str], table_details: Dict[str, TableInfo]
    ) -> Dict[str, Any]:
        """分析缺失表的严重程度"""
    ...
    def _generate_recommendations(
        self, categorized_missing: Dict[str, List[str]], severity: str
    ) -> List[str]:
        """生成建议和操作指南"""
        recommendations = []
        ...
        if severity == "critical":
            ...
            if categorized_missing["critical"]:
                recommendations.append(
                    f"🎯 缺失的核心表: {', '.join(categorized_missing['critical'])}"
                )
            ...
            if categorized_missing["important"]:
                recommendations.append(
                    f"📋 缺失的重要表: {', '.join(categorized_missing['important'])}"
                )
    ...
    def _log_validation_result(self, result: Dict[str, Any]):
        ...
        if result["complete"]:
            ...
            self.logger.warning(
                f"⚠️ 备份完整性验证发现问题 - 严重程度: {result['severity']}"
            )
            self.logger.warning(
                f"缺失 {result['missing_count']} 张表: {result['missing_tables']}"
            )
