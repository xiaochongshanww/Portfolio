...
class SimpleRestoreEngine:
    ...
    def _setup_logger(self) -> logging.Logger:
        ...
        log_file_path = (
            ...
            / "logs"
            / "simple_restore_engine"
            / "simple_restore_engine.log"
        )
    ...
    def restore_database(
        self, backup_file_path: str, restore_id: str
    ) -> Dict[str, Any]:
        """执行数据库恢复 - 核心方法"""
        ...
        try:
            ...
            with tempfile.TemporaryDirectory(
                prefix=f"restore_{restore_id}_"
            ) as temp_dir:
    ...
    def _try_docker_restore_with_copy(
        self, container_name: str, sql_file: Path
    ) -> Dict[str, Any]:
        ...
        try:
            ...
            if copy_process.returncode != 0:
                return {
                    "success": False,
                    "error": f"文件复制失败: {copy_process.stderr}",
                }
            ...
            try:
                ...
                if copy_filtered_process.returncode != 0:
                    return {
                        "success": False,
                        "error": f"过滤文件复制失败: {copy_filtered_process.stderr}",
                    }
            ...
            restore_cmd = ["docker", "exec", container_name, "sh", "-c", mysql_cmd]

            self.logger.info(
                f"执行命令: docker exec {container_name} sh -c 'mysql ... --execute=SOURCE filtered_file'"  # noqa: E501
            )
            ...
            if restore_process.returncode == 0:
                ...
                if is_acceptable:
                    self.logger.warning(
                        f"Docker文件复制恢复部分成功（存在兼容性警告）: {restore_process.stderr}"
                    )
                    return {
                        "success": True,
                        "method": "docker_protected_restore",
                        "warnings": restore_process.stderr,
                    }
                else:
    ...
    def _try_docker_restore_with_stdin(
        self, container_name: str, sql_file: Path
    ) -> Dict[str, Any]:
        ...
        try:
            ...
            if process.returncode == 0:
                ...
                if is_acceptable:
                    self.logger.warning(
                        f"Docker stdin恢复部分成功（存在兼容性警告）: {process.stderr}"
                    )
                    return {
                        "success": True,
                        "method": "docker_stdin_protected",
    ...
    def _try_system_mysql_restore(self, sql_file: Path) -> Dict[str, Any]:
        ...
        try:
            ...
            if process.returncode == 0:
                ...
                stderr_content = process.stderr.lower()
                is_acceptable = (
                    "doesn't exist" in stderr_content
                    or "unknown table" in stderr_content
                )

                if is_acceptable:
                    self.logger.warning(
                        f"系统mysql恢复部分成功（存在兼容性警告）: {process.stderr}"
                    )
                    return {
                        "success": True,
                        "method": "system_mysql_protected",
                        "warnings": process.stderr,
                    }
                else:
    ...
    def _filter_protected_tables_in_container(
        self, container_name: str, input_file: str, output_file: str
    ) -> Dict[str, Any]:
        """在Docker容器内过滤受保护的系统表"""
        try:
            # 定义受保护的系统表
            ...
            filter_cmd = ["docker", "exec", container_name, "sh", "-c", sed_cmd]
...
"""
            ...
            for dep_table, dependencies in dependent_tables.items():
                self.logger.info(
                    f"处理依赖表 {dep_table}，移除所有外键约束和DROP语句以避免创建顺序问题"
                )

                # 1. 移除该表的DROP TABLE语句（避免DROP不存在的表导致错误）
                drop_pattern = rf"DROP TABLE IF EXISTS `{re.escape(dep_table)}`.*?;"
                drop_count = len(
                    re.findall(drop_pattern, sql_content, re.IGNORECASE | re.DOTALL)
                )
                sql_content = re.sub(
                    drop_pattern,
                    f"-- DROP TABLE `{dep_table}` removed for safety",
                    sql_content,
                    flags=re.IGNORECASE | re.DOTALL,
                )

                # 2. 移除该表的所有外键约束（不管是否指向受保护表）
                constraint_pattern = rf"CONSTRAINT `{re.escape(dep_table)}_ibfk_\d+` FOREIGN KEY \([^)]+\) REFERENCES `[^`]+` \([^)]+\),?\s*"  # noqa: E501
                removed_count = len(
                    re.findall(constraint_pattern, sql_content, re.IGNORECASE)
                )
                sql_content = re.sub(
                    constraint_pattern, "", sql_content, flags=re.IGNORECASE
                )

                # 3. 移除该表的LOCK TABLES语句（避免锁定不存在的表）
                lock_pattern = rf"LOCK TABLES `{re.escape(dep_table)}`.*?;"
                lock_count = len(
                    re.findall(lock_pattern, sql_content, re.IGNORECASE | re.DOTALL)
                )
                sql_content = re.sub(
                    lock_pattern,
                    f"-- LOCK TABLE `{dep_table}` removed for safety",
                    sql_content,
                    flags=re.IGNORECASE | re.DOTALL,
                )

                # 4. 清理可能产生的多余逗号（如果外键约束在最后一个字段后）
                cleanup_pattern = r"(,\s*)\s+\) ENGINE="
                sql_content = re.sub(
                    cleanup_pattern, r"\n) ENGINE=", sql_content, flags=re.IGNORECASE
                )

                self.logger.info(
                    f"已移除 {dep_table} 的 {drop_count} 个DROP语句、{lock_count} 个LOCK语句和 {removed_count} 个外键约束"  # noqa: E501
                )
            ...
            for table in protected_tables:
                ...
                sql_content = re.sub(
                    drop_pattern, "", sql_content, flags=re.IGNORECASE | re.DOTALL
                )

                # 过滤完整的CREATE TABLE块（从CREATE到分号结束，包括所有MySQL注释）
                # 匹配可能的MySQL client设置
                mysql_client_start = (
                    r"/\*!40101 SET @saved_cs_client\s*=\s*@@character_set_client \*/;"
                )
                mysql_client_end = (
                ...
                sql_content = re.sub(
                    create_pattern, "", sql_content, flags=re.IGNORECASE | re.DOTALL
                )

                # 简化版：直接匹配CREATE TABLE到分号
                simple_create_pattern = rf"CREATE TABLE `{re.escape(table)}`.*?;"
                sql_content = re.sub(
                    simple_create_pattern,
                    "",
                    sql_content,
                    flags=re.IGNORECASE | re.DOTALL,
                )
                ...
                sql_content = re.sub(
                    lock_pattern, "", sql_content, flags=re.IGNORECASE | re.DOTALL
                )

                # 过滤INSERT INTO语句（可能跨越多行）
                insert_pattern = rf"INSERT INTO `{re.escape(table)}`.*?;"
                sql_content = re.sub(
                    insert_pattern, "", sql_content, flags=re.IGNORECASE | re.DOTALL
                )

                # 过滤相关的MySQL设置语句
                client_pattern = r"/\*!40101 SET @saved_cs_client\s*=\s*@@character_set_client \*/;.*?/\*!40101 SET character_set_client = @saved_cs_client \*/;"  # noqa: E501
                sql_content = re.sub(
                    client_pattern, "", sql_content, flags=re.IGNORECASE | re.DOTALL
                )
            ...
            self.logger.info("SQL块级过滤完成")
            self.logger.info(
                f"原始行数: {original_lines}, 过滤后行数: {filtered_line_count}"
            )
