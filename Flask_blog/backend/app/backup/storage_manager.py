...
class LocalStorageProvider(StorageProvider):
    ...
    def __init__(self, base_dir: str = None):
        self.base_dir = Path(
            base_dir or current_app.config.get("LOCAL_BACKUP_DIR", "backups/local")
        )
...
class AliyunOSSProvider(StorageProvider):
    ...
    def __init__(
        self, bucket_name: str, access_key: str, secret_key: str, endpoint: str
    ):
        self.bucket_name = bucket_name
        self.access_key = access_key
        self.secret_key = secret_key
    ...
    def bucket(self):
        ...
        if self._bucket is None:
            try:
                ...
                raise ImportError(
                    "oss2 is required for Aliyun OSS storage. Install with: pip install oss2"  # noqa: E501
                )
        return self._bucket
    ...
    def client(self):
        ...
        if self._client is None:
            try:
                ...
                raise ImportError(
                    "cos-python-sdk-v5 is required for Tencent COS storage."
                )
        return self._client
        ...
        try:
            ...
            with open(file_path, "rb") as f:
                response = self.client.put_object(
                    Bucket=self.bucket_name, Body=f, Key=key
                )
    ...
    def __init__(self, key: bytes = None):
        if not CRYPTOGRAPHY_AVAILABLE:
            current_app.logger.warning(
                "Cryptography library not available. Backup encryption disabled."
            )
            self.enabled = False
            return
        ...
        if key:
            ...
            if key_str:
                ...
                current_app.logger.warning(
                    "Generated new encryption key. Save it securely!"
                )
...
class StorageManager:
    ...
    def store_backup(
        self, file_path: Path, backup_id: str, providers: list = None
    ) -> Dict[str, Any]:
        """存储备份到多个后端"""
        ...
        try:
            for provider_name in providers:
                if provider_name in self.providers:
                    try:
                        result = self.providers[provider_name].upload(
                            upload_file, backup_id
                        )
                        ...
                        current_app.logger.error(
                            f"Failed to upload to {provider_name}: {e}"
                        )
                else:
                    storage_info[provider_name] = {
                        "status": "error",
    ...
    def retrieve_backup(
        self, backup_id: str, target_path: Path, provider_name: str = None
    ) -> bool:
        """从存储后端恢复备份"""
        providers_to_try = (
        ...
        for provider in providers_to_try:
            if provider in self.providers:
                try:
                    ...
                    if current_app.config.get("BACKUP_ENCRYPTION_ENABLED", True):
                        download_path = (
                            target_path.parent / f"{target_path.name}.encrypted"
                        )
    ...
    def delete_backup(
        self, backup_id: str, storage_info: Dict[str, Any]
    ) -> Dict[str, bool]:
        """删除存储后端的备份文件"""
        results = {}
        ...
        for provider_name, info in storage_info.items():
            if provider_name in self.providers and info.get("status") == "success":
                try:
                    results[provider_name] = self.providers[provider_name].delete(
                        backup_id
                    )
                except Exception as e:
                    current_app.logger.error(
                        f"Failed to delete from {provider_name}: {e}"
                    )
