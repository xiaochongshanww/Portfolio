...
class UltralthinkRestoreManager:
    ...
    def restore_backup(
        ...
        try:
            ...
            restore_record = (
                ...
                .filter_by(id=restore_record_id)
                .first()
            )
            ...
            # 获取对应的备份记录（独立会话）
            backup_record = (
                independent_session.query(BackupRecord)
                .filter_by(id=restore_record.backup_record_id)
                .first()
            )
        ...
        try:
            current_app.logger.info(
                f"开始ULTRALTHINK恢复，恢复id {restore_id}，恢复数据库id {restore_record_id}"
            )

            # 更新状态为运行中（使用记录ID）
            self._safe_update_status_by_id(
                restore_record_id, "running", "开始执行恢复..."
            )
            ...
            try:
                ...
                if not validation_result["can_proceed_safely"]:
                    ...
                    validation_result.get("missing_tables", [])
                    ...
                    current_app.logger.error(
                        f"建议: {validation_result.get('recommendations', [])}"
                    )

                    # 更新状态为失败
                    self._safe_update_status_by_id(
                        restore_record_id, "failed", error_msg
                    )
                    ...
                    current_app.logger.warning(
                        f"⚠️ 发现表缺失 ({validation_result['severity']} 级别)，但允许继续恢复"
                    )
                    current_app.logger.warning(
                        f"缺失表: {validation_result.get('missing_tables', [])}"
                    )
            ...
            if result["success"]:
                ...
                self._safe_update_status_by_id(
                    restore_record_id, "failed", f"恢复失败: {error_msg}"
                )
                return {
                    "status": "failed",
                    "error": error_msg,
            ...
            try:
                self._safe_update_status_by_id(
                    restore_record_id, "failed", f"恢复异常: {str(e)}"
                )
            except Exception:
                pass  # 避免级联异常
            return {
    ...
    def _execute_independent_restore(
        self, backup_file: Path, restore_id: str
    ) -> Dict[str, Any]:
        """执行独立进程恢复 - 核心架构改进"""
        try:
            ...
            # 构建命令
            cmd = [python_exe, str(restore_engine_path), str(backup_file), restore_id]
    ...
    def _safe_update_status(
        self, restore_record: RestoreRecord, status: str, message: str
    ):
        """安全更新恢复状态 - 会话隔离版本"""
        max_retries = 3
        for attempt in range(max_retries):
            try:
                ...
                current_app.logger.warning(
                    f"状态更新失败 (尝试 {attempt + 1}/{max_retries}): {e}"
                )
    ...
    def _safe_update_status_by_id(self, record_id: int, status: str, message: str):
        ...
        current_app.logger.info(
            f"状态更新请求 - 记录ID: {record_id}, 状态: {status}, 消息: {message}"
        )
        max_retries = 3
        for attempt in range(max_retries):
    ...
    def cancel_restore(self, restore_id: str) -> bool:
        """取消恢复任务 - 简化版本"""
        try:
            restore_record = RestoreRecord.query.filter_by(
                restore_id=restore_id
            ).first()
            if not restore_record:
                return False
    ...
    def _verify_backup_completeness(self, backup_file: Path) -> Dict[str, Any]:
        ...
        try:
            ...
            with tempfile.TemporaryDirectory(prefix="validation_") as temp_dir:
                ...
                current_app.logger.info(
                    f"ULTRALTHINK智能验证完成 - 完整性: {validation_result['complete']}"
                )
    ...
    def get_validation_report(self, backup_file: Path) -> Dict[str, Any]:
        ...
        try:
            ...
            return {"status": "error", "error": str(e), "message": "获取验证报告失败"}
