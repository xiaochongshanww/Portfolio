...
def get_backup_records():
    ...
    total, items = list_backup_records(
        page, size, status, backup_type, sort_by, sort_order
    )
    return _ok(
        {
            "total": total,
...
def create_backup():
    ...
    try:
        ...
        if result.get("success"):
            sync_physical_backups_to_database()
            return _ok(
                {"backup_id": result.get("backup_id"), "status": "started"},
                "Backup started",
            )
...
def download_backup(backup_id):
    try:
        ...
        return send_file(
            backup_path, as_attachment=True, download_name=f"backup_{backup_id}.tar.gz"
        )
    except Exception as e:
        return _err(5000, str(e), 500)
...
def restore_backup(backup_id):
    ...
    try:
        ...
        if result.get("success"):
            ...
            db.session.commit()
            return _ok(
                {"restore_id": restore_record.id, "status": "completed"},
                "Restore completed",
            )
...
# [rest of the routes: sync, config, statistics, cleanup, delete, tasks, physical, external, restores...]  # noqa: E501
# ─── 完整保留现有路由以保持兼容性 ──────────────────────────
...
def create_physical_backup():
    try:
        ...
        if result.get("success"):
            sync_physical_backups_to_database()
            return _ok(
                {"backup_id": result.get("backup_id")}, "Physical backup created"
            )
        return _err(5000, result.get("error", "Backup failed"), 500)
    except Exception as e:
        return _err(5000, str(e), 500)
