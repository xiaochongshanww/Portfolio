...
class TaskCleaner:
        ...
        self.config.update(
            {
                ...
                "restore_timeout_minutes": app.config.get(
                    "RESTORE_TIMEOUT_MINUTES", 30
                ),
                "cleanup_interval_minutes": app.config.get(
    ...
    def start_cleanup_daemon(self):
        ...
        self.cleaning_thread = threading.Thread(
            target=self._cleanup_daemon_worker, name="TaskCleanupDaemon", daemon=True
        )
    ...
    def cleanup_stuck_tasks(self) -> Dict[str, Any]:
        ...
        try:
            ...
            result["total_cleaned"] = (
                result["backup_cleaned"] + result["restore_cleaned"]
            )
        ...
        for attempt in range(max_retries + 1):
            try:
                ...
                if attempt < max_retries:
                    ...
                    delay = 2**attempt
                    self.logger.debug(
                        f"{operation_name} 数据库连接失败 (尝试 {attempt + 1}/{max_retries + 1})，{delay}秒后重试: {e}"  # noqa: E501
                    )
                    time.sleep(delay)
                else:
    ...
    def _cleanup_stuck_backups(self) -> Dict[str, Any]:
                ...
                try:
                    ...
                    if backup.file_path and backup.file_path.startswith("backups/"):
                        ...
                        if os.path.exists(full_path):
                            ...
                            self.logger.info(
                                f"修复备份任务状态: {backup.backup_id} -> completed"
                            )
                        else:
                            # 文件不存在，标记为失败
        ...
        operation_result = self._safe_database_operation(
            backup_cleanup_operation, "清理备份任务", max_retries=2
        )

        if operation_result["success"]:
            return operation_result["result"]
        else:
            return {"cleaned_count": 0, "errors": [operation_result["error"]]}
    ...
    def _cleanup_stuck_restores(self) -> Dict[str, Any]:
        ...
        # 使用安全包装器执行操作
        operation_result = self._safe_database_operation(
            restore_cleanup_operation, "清理恢复任务", max_retries=2
        )
