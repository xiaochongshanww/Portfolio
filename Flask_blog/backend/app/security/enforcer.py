...
# 注册错误码（如需动态扩展）
def register_error(
    code: int, message: str, http_status: int = 400, description: Optional[str] = None
):
    _ERROR_REGISTRY[code] = {
        "message": message,
...
def workflow_transition(
    entity_loader: Callable[..., object],
    target_status: str,
    status_attr: str = "status",
    lock_attr: str = "updated_at",
    inject_name: str = "article",
):
        ...
        def wrapper(*args, **kwargs):
            ...
            if inject_name in kwargs:
                ...
                raise BusinessError(
                    ...
                    "internal_error",
                    500,
                    data={"reason": "inject name collision"},
                )
