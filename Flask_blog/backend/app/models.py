...
class Comment(db.Model):
    ...
    id = db.Column(db.Integer, primary_key=True)
    article_id = db.Column(
        db.Integer, db.ForeignKey("articles.id"), index=True, nullable=False
    )
...
class AuditLog(db.Model):
    ...
    id = db.Column(db.Integer, primary_key=True)
    article_id = db.Column(
        db.Integer, db.ForeignKey("articles.id"), index=True, nullable=True
    )
    ...
    )  # submit/approve/reject/unpublish/schedule/unschedule/delete/rollback
...
class BackupConfig(db.Model):
    ...
    category = db.Column(
        db.String(50), default="general", index=True
    )  # general, storage, schedule, security
...
class BackupTask(db.Model):
    ...
    # 备份配置
    backup_type = db.Column(
        db.String(20), default="incremental"
    )  # full, incremental, snapshot
...
class RestoreRecord(db.Model):
    ...
    backup_record_id = db.Column(
        db.Integer, db.ForeignKey("backup_records.id"), nullable=True
    )  # 物理恢复时可为空

    # 恢复配置
    restore_type = db.Column(
        db.String(20), nullable=False
    )  # full, partial, database_only, files_only
...
class MediaFolder(db.Model):
    ...
    children = db.relationship(
        "MediaFolder", backref=db.backref("parent", remote_side=[id]), lazy="dynamic"
    )
    ...
    def to_dict(self):
        ...
        return {
            ...
            "owner_name": (
                self.owner.nickname or self.owner.email if self.owner else None
            ),
...
class MediaUsage(db.Model):
    ...
    usage_type = db.Column(
        db.Enum("article_content", "article_featured", "user_avatar", "other"),
        nullable=False,
        index=True,
    )
