...
from .decorators import (  # noqa: E402, F401 — re-exported for existing imports
    require_auth,
    require_roles,
)
    ...
    if log_format == "json":
        ...
        class HumanFormatter(logging.Formatter):
            ...
            def format(self, record):
                ...
                return f"{time_str} {colored_level} {logger_name:<20} {message}{extra_info}"  # noqa: E501
    ...
    try:
        if os.getenv("LOG_FILE_ENABLED", "true").lower() == "true":
            ...
            has_file_handler = any(
                isinstance(h, (TimedRotatingFileHandler, RotatingFileHandler))
...
def _setup_cors(app):
    ...
    if cors_origins == "*" and app.config.get("ENV") == "production":
        app.logger.warning(
            "CORS_ORIGINS='*' in production — restricting to same-origin"
        )
        cors_origins = []
    CORS(
        app,
...
def _setup_limiter(app):
    ...
    def _inject_security_headers(resp):
        ...
        csp = "default-src 'self'; script-src 'self' 'unsafe-inline' 'unsafe-eval' cdnjs.cloudflare.com cdn.jsdelivr.net; style-src 'self' 'unsafe-inline' cdnjs.cloudflare.com cdn.jsdelivr.net; img-src 'self' data: blob: https:; font-src 'self' data: cdnjs.cloudflare.com; connect-src 'self' https:; media-src 'self'; frame-src 'self' https://www.youtube.com https://www.bilibili.com; object-src 'none'"  # noqa: E501
...
def _setup_scheduler(app):
    if getattr(app.config, "ENABLE_SCHEDULER", False) and BackgroundScheduler:
        ...
        scheduler.add_job(
            publish_scheduled,
            "interval",
            seconds=app.config.get("SCHEDULE_CHECK_INTERVAL", 60),
...
def _setup_metrics(app):
    ...
    if (
        not METRICS_ENABLED
        or not os.getenv("PROMETHEUS_ENABLED", "true").lower() == "true"
    ):
        METRICS_ENABLED = False
        return
    try:
