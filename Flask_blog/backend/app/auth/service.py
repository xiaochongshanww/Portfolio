...
def refresh_tokens(
    refresh_token: str, xsrf_cookie: str, xsrf_header: str
) -> tuple[str, str] | None:
    ...
    try:
        ...
        if rc:
            current_jti = (
                rc.get(f"refresh:current:{sub}") if hasattr(rc, "get") else None
            )
            if (
                rc.get(f"refresh:blacklist:{jti}")
                or not rc.get(f"refresh:allow:{jti}")
                or (current_jti and current_jti != jti)
            ):
                return None
    except Exception:
        return None
    ...
    if rc:
        try:
            ...
            try:
                ...
                store = getattr(rc, "store", {})
                user_keys = [
                    k for k in store.keys() if k.startswith(f"refresh:user:{user.id}:")
                ]
