...
def _make_security_event(
    event_type, description, user_id=None, severity="low", source_ip=None, extra=None
):
    """记录安全事件（容错）。"""
    try:
...
def login():
    ...
    _make_security_event(
        ...
        extra={
            "role": user.role,
            "user_agent": (request.headers.get("User-Agent", "")[:100] or ""),
        },
    )
    ...
    resp = make_response(
        jsonify(
            {
                "code": 0,
                "data": {"access_token": access, "role": user.role},
                "message": "ok",
            }
        )
    )
...
def refresh():
    ...
    resp = make_response(
        jsonify({"code": 0, "data": {"access_token": new_access}, "message": "ok"})
    )
