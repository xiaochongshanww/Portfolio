...
>
> **实测现状(2026-08-15)**:
> - 测试:后端 300+ 全绿、**覆盖率 74.65%**(门槛 50%);前端 20/20→171 测试、**覆盖率 81.19%**(全应用口径,门槛 24)。
...
> - 剩余差距(诚实清单):前端覆盖率仍低(7 spec/37 视图);后端覆盖率低于自定 80% 标准;mypy 全量 0 错误(类型债清零,仅剩第 3 方库 import-untyped 与 flask-sqlalchemy db.Model 的定向豁免);`tests_manual/` 4 个手动脚本;lhci 非阻断。
