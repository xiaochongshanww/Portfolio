...
import { useArticleForm } from '../composables/useArticleForm.js';
import { useArticleAutoSave } from '../composables/useArticleAutoSave.js';
import { useArticleShortcuts } from '../composables/useArticleShortcuts.js';
...
// 表单与校验(组合式,05 §24)
const { form, formErrors, showValidation, validateField, validateForm, clearFieldError, handleFieldBlur } = useArticleForm();
...
// 自动保存与本地草稿(组合式,05 §25)
const { autoSaving, lastSaveTime, hasUnsavedChanges, isRestoringDraft, formatSaveTime, triggerAutoSave, saveDraft, loadLatestDraft, handleBeforeUnload } = useArticleAutoSave({ form, blockEditorRef });

// 键盘快捷键(组合式;submit 为函数声明,存在提升)
const { handleKeyDown, showKeyboardShortcuts } = useArticleShortcuts({ saveDraft, submit, loading, error });
// 工具函数
