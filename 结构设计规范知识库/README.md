...
完整验证会为三类评估报告写入同一 `verification_run_id`，并由实际 API 进程写入基于非敏感配置和关键实现的 `runtime_config_hash`。发布门禁要求三份报告来自同一次运行且匹配当前运行指纹；单项执行、旧契约或其他检索/模型配置的报告不能拼接成通过结论。
...
快照只保留报告时间、状态、失败检查、相对路径、SHA-256 和评估集数量，不包含问题、回答、错误详情、秘密或本机绝对路径。指针存在时，`--write` 会先独立校验运行清单和全部十个产物，再从同一运行提取验证与门禁摘要；旧环境无指针时才读取兼容文件。随后它把同一脱敏快照幂等归档到 `docs/quality/质量证据历史/`，重建 `质量证据历史索引.json`，最后更新当前状态；CI 会验证历史指纹、索引、当前快照、评估集和系统卡。在受控本机存在源报告时，同一校验命令还会核对源报告哈希。该历史提供 Git 内部追溯，不是数字签名或外部可信存证。
