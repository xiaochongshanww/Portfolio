...
`DB_DIR` 是无活动指针时的基础 Chroma 路径配置；当前在线检索、管理端解析文本和图片接口优先依据 `data/active_db.json` 定位活动版本。活动指针中的数据库、解析文本、图片、MinerU、审计和 manifest 路径相对于项目根目录保存，读取端兼容历史 Windows/Posix 绝对路径及缺少扩展目录字段的旧指针。部署时不得只迁移 `DB_DIR` 而忽略活动指针和整个版本目录。
...
`package-probe` 在临时目录执行真实导入，并用独立 Python 子进程打开 Chroma、核对集合数量和读取样本。子进程用于隔离 Chroma 原生索引状态与 Windows 文件锁，探测完成后不会改变项目活动指针。正式导入默认拒绝已安装版本和内容不同的同名资产；`--replace` 只能在已备份并确认覆盖范围后使用。`--no-activate` 可用于预安装和验证，不改变当前活动指针。
...
`Dockerfile` 使用三个阶段：Node 22 构建 Vue 控制台、Python 3.11 按哈希安装运行锁、最终 Python 3.11 运行镜像。Python 直接依赖由 `.in` 维护，`dependency-lock.json` 固定 `uv`、Python 和依赖时间截点，CI 从临时目录重新解析以阻止锁漂移。最终镜像包含 `src/`、`frontend/dist`、评估集和规范元数据，不包含 Node.js、MinerU、原始 PDF 或已构建知识库。
...
当前质量体系分两层：管理端重建的候选激活门禁强制执行运行时一致性、常规检索和结构化表专项评估；完整发布门禁在此基础上额外要求与活动数据版本一致的回答级盲测、报告新鲜度和任务状态。最近完整回答级证据见 `data/audit/reports/verification_latest.json`，生成时间为 2026-07-05；其结论不能替代本次或未来发布的重新验证。

### 8.3 回退顺序

1. 暂停受影响的公网入口或切换到已知稳定版本。
2. 恢复 `active_db.json` 指向的完整运行版本和对应根 manifest；版本化重建产物中的数据库、解析文本和图片随指针一并恢复。
3. 必要时恢复独立管理的结构化表发布版本、批准修正和原始 PDF。
...
2. 为失败候选和历史活动版本定义自动保留数量、磁盘水位和受审计清理策略。
3. 在产品交付形态确定后，将已建立的 Windows/Linux x86_64 知识包兼容矩阵扩展到目标版本、架构和实际用户环境。
4. 在需要多 worker 或多副本前，引入跨进程活动版本协调、外部指标采集、集中日志和明确的数据保留策略。
5. 为服务端引入用户、角色、租户和审计主体模型前，保持服务范围为受控小规模试用。
