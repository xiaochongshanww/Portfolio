...
本文不将以下能力表述为已完成：生产级 Docker 交付、自动发布阻断、原始 PDF 或页面截图的受控访问、租户隔离、账号体系、版权授权台账。
...
`DB_DIR` 是基础 Chroma 路径配置；当前在线检索优先依据 `data/active_db.json` 定位活动版本。部署时不得只迁移 `DB_DIR` 而忽略活动指针、版本 manifest 和关联数据资产。
...
sequenceDiagram
    ...
    A-->>O: 返回已激活版本
    Note over O,R: 当前流程不自动运行或阻断质量门禁
    O->>A: 分别运行评估与质量门禁
    alt 门禁失败
        O->>A: 暂停发布并人工回退数据与表资产
    end
...
当前 `Dockerfile` 仅复制 `src/` 和运行时 Python 依赖；`.dockerignore` 排除 `data/`，镜像不会携带前端产物或知识库运行数据。现有 Compose 仅挂载 `db`、`logs` 与 `data/images`，未覆盖原始 PDF、活动数据库版本、manifest、结构化表和审计数据。

因此，Docker Compose 当前仅可视为 API/Open WebUI 实验性运行方案，不能作为完整控制台与知识库的生产交付。要成为可交付方案，至少需要：

1. 明确构建或挂载 `frontend/dist`。
2. 为 `data/raw`、`data/db_versions`、`data/active_db.json`、manifest、结构化表、修正与审计数据定义持久卷策略。
3. 在空环境完成重建、重启、页面图片、回退和鉴权验证。
4. 通过反向代理提供 TLS、访问控制和日志策略。
