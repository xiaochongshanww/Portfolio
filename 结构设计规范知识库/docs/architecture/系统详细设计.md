...
> 维护角色：工程负责人  
> 文档更新：2026-08-23
> 代码核对：2026-08-21（embedding-3 活动版本和独立向量索引已核对）
> 完整运行验证：2026-08-19 UTC（运行 `5734ee7b99e24203bb657dbb20f271bc` 完成 612 项后端测试、前端生产构建、100/12/24 三类评估和自动质量门禁；真实内容来源权利、公网部署和封闭试用尚未验证）
> 当前代码回归：2026-08-21，隔离开发依赖完成 `671 passed, 1 skipped`；这是当前代码回归，不替代上述质量运行或外部发布证据
> 验证证据：[运行容器非 root 身份验证记录](../releases/运行容器非root身份验证记录.md)、[Docker 构建上下文最小化验证记录](../releases/Docker构建上下文最小化验证记录.md)、[已安装依赖图一致性门禁验证记录](../releases/已安装依赖图一致性门禁验证记录.md)、[测试客户端 HTTPX2 迁移验证记录](../releases/测试客户端HTTPX2迁移验证记录.md)、[控制台供应商能力诊断验证记录](../releases/控制台供应商能力诊断验证记录.md)、[质量验证供应商能力预检验证记录](../releases/质量验证供应商能力预检验证记录.md)、[管理 API 前端生成客户端验证记录](../releases/管理API前端生成客户端验证记录.md)、[质量证据原子发布验证记录](../releases/质量证据原子发布验证记录.md)、[环境自检验证记录](../releases/环境自检验证记录.md)、[配置示例可执行验证记录](../releases/配置示例可执行验证记录.md)
...
| 常规检索 | `src/app/retrieval/` | 独立精确向量矩阵、BM25、条文、权威与表格意图基线排序，按配置扩展精排候选池 | 智谱 Embedding、Chroma 记录层、NumPy、BM25 |
...
flowchart LR
    ...
    G --> T["类型 + Fetch client + 逐操作 SDK"]
    T --> A["admin-api.ts 语义别名"]
    A --> V["Vue 管理组件"]
```

FastAPI 路由与 Pydantic 模型是接口语义的唯一权威来源。45 个 JSON 管理操作的 HTTP 200 响应引用具名模型；页面截图操作声明 `image/png`。响应默认拒绝未建模顶层字段，只有 manifest、原始候选/人工队列/草稿、AI 建议、批准修正文件和单元素七类登记载荷允许扩展。导出器在不启动服务器、不读取模型凭据和不修改运行数据的条件下生成排序稳定的 UTF-8 快照，并拒绝空 schema、重复 operation id 和错误媒体类型。

前端从快照生成类型、Fetch client 和逐操作 SDK。组件使用结构化 `path`、`query`、`body` 参数，方法、模板路径和传输类型由同一个生成操作绑定；`admin-api.ts` 只为生成函数提供语义别名，不重新声明路径或模型。`api.ts` 通过 client 拦截器保留同源基址、Bearer Key、候选 Key 首次验证、统一 `ApiError` 和 401 全局事件；页面截图由生成操作解析为 Blob 后创建对象 URL。`contracts.ts` 只保留控制台展示视图，不再承担传输路径映射。`api:check` 除重生成差异外，还拒绝旧管理包装器和运行时代码中的 `/admin` 字面量。具体维护流程见[契约规范](../reference/管理API与前端类型契约规范.md)，决策见 [ADR 0030](../adr/0030-前端管理调用采用OpenAPI生成客户端.md)。

静态契约之后还有运行时实现门禁。`tests/test_admin_runtime_contract.py` 从当前 OpenAPI 取得完整管理操作集合，以 operation id 对照 46 个隔离场景，并通过 `TestClient` 执行请求解析、路由、中间件、响应模型和序列化。JSON 响应按 OpenAPI `$ref` 复验 Pydantic 模型，页面响应核对 PNG 媒体类型和签名；临时目录与确定性替身保证测试不触碰真实运行数据、模型、MinerU 或后台线程。缺少场景、重复/未知场景、方法或媒体类型漂移，以及严格响应的缺失/额外字段都会失败关闭。决策见 [ADR 0029](../adr/0029-管理API采用清单驱动的运行时契约测试.md)。
...
概览页初始状态只显示“尚未检测”，挂载、父级数据刷新和全局刷新均不调用模型供应商。操作者点击“检测”后，前端通过生成 SDK 调用受保护的 `POST /admin/provider-probes`；按钮在请求期间禁用，避免同一轮重复提交。端点并行执行一次最小智谱 Embedding 和一次最小 MiMo 聊天请求，只返回提供方、能力、模型、稳定状态、延迟和可选 HTTP 状态。

上游 401、429、超时、5xx、网络失败和无效响应被转换为稳定能力状态，端点本身仍返回结构化成功响应；控制台 API 自身返回 401 时继续进入 3.3 的统一鉴权流程。结果只表示检测时刻的外部调用能力，不持久化为健康状态，不自动轮询，也不进入 `/ready` 或质量报告。详见 [I-041 验证记录](../releases/质量验证供应商能力预检验证记录.md) 与 [I-042 验证记录](../releases/控制台供应商能力诊断验证记录.md)。
...
| 候选/活动版本 | `data/db_versions/{job_id}/` | 重建工作流 | Chroma 记录层、精确向量索引、BM25、校对、图片和审计 | 包含 `db`（含 `dense_vectors.npy/json`）、`processed`、`images`、`mineru`、`audit`、`quality` 和 manifest |
| 增量缓存索引 | `data/document_cache/index.json` | 成功激活后的重建工作流 | 下一次重建诊断 | 可重建的派生索引，不是提交点；可信复用来源仍是已通过门禁的活动版本 |
...
运行环境只需 API/前端依赖、问答与 Embedding 凭据，以及导入后的 manifest、Chroma 记录层、版本化精确向量矩阵、可选结构化表和图片。知识包不包含解析器、密钥、日志、候选修正和人工草稿。
...
sequenceDiagram
    ...
    O->>A: 预检并提交重建任务（增量/全量、解析后端、批准修正）
    A->>P: 比对处理契约和逐文档指纹
    alt 契约不兼容或缓存损坏
        P->>P: 自动回退全量解析与向量化
    else 契约兼容
        P->>S: 复用未变化文档产物和稳定 Chunk 向量
        P->>P: 只解析新增/变化文档并生成缺失向量
    end
    A->>P: 在 data/db_versions/{job_id} 组装完整候选产物
    P->>S: 写候选 db/processed/images/mineru/audit/manifest
    A->>R: 独立加载候选 Chroma、精确向量索引与 BM25
...
管理端默认采用增量候选重建。处理契约包含解析器实现与版本、解析参数、解析/Chunk/入库代码摘要、Embedding 模型与维度、精确向量格式和修正开关；逐文档指纹包含 PDF、元数据、批准修正和对应结构化表修订。复用前还会验证解析文件、引用产物和 SHA-256。旧 manifest 无契约、活动向量索引缺失或契约漂移时自动全量回退。无论处理输入是增量还是全量，最终 Chroma 和精确向量索引都是包含全部活动文档的完整候选版本。

重建只清理本次候选版本目录，不在构建阶段删除活动解析文本、图片或数据库。批准修正默认会在候选构建时应用，候选或待审项不会自动进入批准集。候选门禁不调用在线问答 API 或外部 LLM；激活后仍须运行回答级盲测并通过完整发布质量门禁。详细决策见 [ADR 0048](../adr/0048-日常知识重建采用增量处理与完整候选发布.md)。
...
sequenceDiagram
    ...
    B->>P: 导出活动 manifest、Chroma、精确向量索引与可选资产
...
`package-probe` 在临时目录执行真实导入，并用独立 Python 子进程打开 Chroma、核对集合数量和读取样本；API 冷启动时还会按 manifest 校验版本化精确向量矩阵的数量、维度、模型和 ID 集合。子进程用于隔离 Chroma 原生索引状态与 Windows 文件锁，探测完成后不会改变项目活动指针。正式导入默认拒绝已安装版本和内容不同的同名资产；`--replace` 只能在维护窗口、上一版完整包已保留且覆盖范围已确认时使用。`--no-activate` 只预装独立数据库版本，不改变当前活动指针、根 manifest 或共享资产。
...
`--preflight-only` 在完整执行前复用同一 `/ready`、管理端鉴权和受保护的 `POST /admin/provider-probes` 契约。连接既有 API 时探测目标，托管模式仍创建并回收独占 loopback 进程。`/ready` 保持无外部调用的本地就绪语义；供应商端点并行执行最小 Embedding 与聊天调用，只返回脱敏能力状态。任一能力失败时，完整验证同样在三类评估前短路并生成同运行失败占位证据。预检分支不进入测试、评估和质量门禁，也不写任何质量 `latest` 报告，只更新不作为质量证据的托管启动日志。结构化 JSON 的非零退出码表示运行前置条件不成立，不能解释为模型内容质量结论。详见 [ADR 0017](../adr/0017-质量验证采用无副作用前置条件预检.md) 与 [ADR 0031](../adr/0031-质量验证采用真实供应商能力预检.md)。
...
开发锁同时显式安装 HTTPX2 作为 Starlette TestClient 后端；运行锁保留业务代码直接使用的 HTTPX，两者职责不合并。pytest 将 `StarletteDeprecationWarning` 升级为错误，因此开发环境缺少 HTTPX2 并回退旧 HTTPX 时会在测试收集阶段失败。依赖契约测试还验证 HTTPX2 没有被误加为运行或 PDF 解析直接依赖。

精确锁和哈希安装解决“解析结果是否可复现”，`python -m pip check` 解决“当前已安装图是否满足所有声明约束”，两者不能互相替代。双平台后端、产包与跨平台恢复任务都在安装后、核心工作前检查依赖图；Docker 在 Python builder 中执行同一检查，再把站点包复制到主动移除 pip 的最终运行镜像。PDF 解析锁仍由实际知识生产环境安装后检查，常规 CI 不下载重型解析依赖来伪造覆盖。
...
`.dockerignore` 对构建上下文采用默认拒绝：只放行 Dockerfile 实际复制的后端源码、前端生产源码与逐项配置、运行锁、评估集和规范元数据。Dockerfile 不使用宽泛的 `COPY frontend/`，因此本机依赖、测试配置、生成输入、构建产物、环境文件与缓存没有先被放行再依赖补丁排除。`scripts/validate_docker_context.py` 维护唯一审阅允许集，同时解析本地 `COPY` 来源；任何未同步的额外放行或复制源都会在 dependency-lock 任务失败。该控制阻止 Git 忽略但存在于工作目录中的 `.tools`、运行数据库、原始数据、`.runtime_*` 密钥和日志被发送给构建器，不改变 Compose 在运行期挂载完整数据卷的边界。

Python builder 在 `/opt/venv` 安装运行锁并执行已安装图检查，随后移除 venv 的 pip、setuptools 与 wheel；最终阶段继续移除基础镜像安装工具。运行镜像通过非零 `APP_UID`/`APP_GID` 构建专用 `app` 用户，镜像内 `data`、`db`、`logs` 归该用户所有，所有复制完成后才执行 `USER app:app`。Compose 以构建参数对齐 Linux 宿主机绑定卷所有者；CI 使用 Runner 实际 UID/GID 构建，并在独立容器和完整 Compose 栈中验证有效 UID 非 0 与数据目录可写。该控制降低容器逃逸前的权限，但不替代 capabilities、seccomp、只读根文件系统或平台级策略。
