...
`.dockerignore` 对构建上下文采用默认拒绝：只放行 Dockerfile 实际复制的源码、运行锁、评估集和规范元数据，并在前端源码放行后重新排除本机依赖、构建产物、环境文件与缓存。`scripts/validate_docker_context.py` 维护唯一审阅允许集，同时解析本地 `COPY` 来源；任何未同步的额外放行或复制源都会在 dependency-lock 任务失败。该控制阻止 Git 忽略但存在于工作目录中的 `.tools`、运行数据库、原始数据、`.runtime_*` 密钥和日志被发送给构建器，不改变 Compose 在运行期挂载完整数据卷的边界。
