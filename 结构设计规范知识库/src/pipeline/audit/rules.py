...
def audit_elements(
    source_file: str, elements: list[dict[str, Any]], artifacts: list[dict[str, Any]] | None = None
) -> dict[str, Any]:
    ...
    page_values = sorted(
        {int(element.get("page") or 0) for element in elements if int(element.get("page") or 0) > 0}
    )
    ...
    if total and empty_count / total > 0.15:
        findings.append(
            _finding(
                "high_empty_text_ratio",
                "medium",
                "空文本比例偏高",
                ratio=round(empty_count / total, 4),
            )
        )
    ...
    for index, element in enumerate(elements):
        ...
        if (
            element.get("chunk_type") in {"table", "formula", "figure"}
            and not element.get("img")
            and not text.strip()
        ):
            findings.append(
                _finding(
                    "media_without_text_or_image",
                    "high",
                    "媒体元素缺少文本和图片引用",
                    element_index=index,
                )
