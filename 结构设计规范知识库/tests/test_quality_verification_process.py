...
from src.evaluation.runner import DEFAULT_EVAL_PATH
...
def _wait_for_job(base_url: str, job_id: str) -> dict:
    deadline = time.monotonic() + 20
    while time.monotonic() < deadline:
        payload = _request_json(f"{base_url}/admin/jobs/{job_id}")
    ...
    with log_path.open("w", encoding="utf-8") as log_file:
        ...
        try:
            ...
            retrieval_job = _request_json(
                ...
                method="POST",
                payload={"file": str(DEFAULT_EVAL_PATH.resolve()), "top_k": 5},
            )
